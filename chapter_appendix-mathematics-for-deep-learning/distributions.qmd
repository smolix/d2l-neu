...
$$F(x) = \begin{cases} 0 & x < 0, \\ 1-p & 0 \le x < 1, \\ 1 & x >= 1 . \end{cases}$$ {#eq-bernoulli-cdf}
...
Now, let's plot the cumulative distribution function @eq-bernoulli-cdf.
...
$$F(x) = \begin{cases} 0 & x < 1, \\ \frac{k}{n} & k \le x < k+1 \textrm{ with } 1 \le k < n, \\ 1 & x >= n . \end{cases}$$ {#eq-discrete-uniform-cdf}
...
Now, let's plot the cumulative distribution function @eq-discrete-uniform-cdf.
...
$$p(x) = \begin{cases} \frac{1}{b-a} & x \in [a, b], \\ 0 & x \not\in [a, b].\end{cases}$$ {#eq-cont-uniform-pdf}

The cumulative distribution function is 

$$F(x) = \begin{cases} 0 & x < a, \\ \frac{x-a}{b-a} & x \in [a, b], \\ 1 & x >= b . \end{cases}$$ {#eq-cont-uniform-cdf}

Let's first plot the probability density function @eq-cont-uniform-pdf.
...
Now, let's plot the cumulative distribution function @eq-cont-uniform-cdf.
...
$$F(x) = \begin{cases} 0 & x < 0, \\ \sum_{m \le k} \binom{n}{m} p^m(1-p)^{n-m}  & k \le x < k+1 \textrm{ with } 0 \le k < n, \\ 1 & x >= n . \end{cases}$$ {#eq-binomial-cdf}
...
Now, let's plot the cumulative distribution function @eq-binomial-cdf.
...
$$X^{(n)} \sim \textrm{Binomial}(n, p/n).$$ {#eq-eq-poisson-approx}

Consider these random variables.  By the previous section, we know that @eq-eq-poisson-approx has mean $\mu_{X^{(n)}} = n(p/n) = p$, and variance $\sigma_{X^{(n)}}^2 = n(p/n)(1-(p/n)) = p(1-p/n)$.  If we take $n \rightarrow \infty$, we can see that these numbers stabilize to $\mu_{X^{(\infty)}} = p$, and variance $\sigma_{X^{(\infty)}}^2 = p$.  This indicates that there *could be* some random variable we can define in this infinite subdivision limit.  
...
$$p_k = \frac{\lambda^ke^{-\lambda}}{k!}.$$ {#eq-poisson-mass}
...
$$F(x) = \begin{cases} 0 & x < 0, \\ e^{-\lambda}\sum_{m = 0}^k \frac{\lambda^m}{m!} & k \le x < k+1 \textrm{ with } 0 \le k. \end{cases}$$ {#eq-poisson-cdf}

Let's first plot the probability mass function @eq-poisson-mass.
...
Now, let's plot the cumulative distribution function @eq-poisson-cdf.
...
$$p_X(x) = \frac{1}{\sqrt{2\pi\sigma^2}}e^{-\frac{(x-\mu)^2}{2\sigma^2}}.$$ {#eq-gaussian-pdf}

Let's first plot the probability density function @eq-gaussian-pdf.
...
Keen-eyed readers will recognize some of these terms.  Indeed, we encountered this integral in @sec-integral-calculus.  Indeed we need exactly that computation to see that this $p_X(x)$ has total area one and is thus a valid density.
...
## Exponential Family {#sec-exponential-family}
...
$$p(\mathbf{x} \mid \boldsymbol{\eta}) = h(\mathbf{x}) \cdot \exp \left( \boldsymbol{\eta}^{\top} \cdot T(\mathbf{x}) - A(\boldsymbol{\eta}) \right)$$ {#eq-exp-pdf}
...
function*, which ensures that the above distribution @eq-exp-pdf 
...
As we allude to in @sec-softmax-and-derivatives, a widely used 
