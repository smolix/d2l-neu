# Eigendecompositions {#sec-eigendecompositions}
...
$$(\mathbf{A} - \lambda \mathbf{I})\mathbf{v} = 0.$$ {#eq-eigvalue-der}

For @eq-eigvalue-der to happen, we see that $(\mathbf{A} - \lambda \mathbf{I})$
...
$$\mathbf{A} = \mathbf{W} \boldsymbol{\Sigma} \mathbf{W}^{-1}.$$ {#eq-eig-decomp}
...
One nice thing about eigendecompositions @eq-eig-decomp is that
...
Thus, in this special case, we can write @eq-eig-decomp as
...
$$[a_{11}-r_1, a_{11}+r_1] = [0.7, 1.3], $$ {#eq-eigendecomposition-auto-1}

$$[a_{22}-r_2, a_{22}+r_2] = [2.4, 3.6], $$ {#eq-eigendecomposition-auto-2}

$$[a_{33}-r_3, a_{33}+r_3] = [4.2, 5.8], $$ {#eq-eigendecomposition-auto-3}

$$[a_{44}-r_4, a_{44}+r_4] = [8.1, 9.9]. $$ {#eq-eigendecomposition-auto-4}
...
for finding the largest eigenvalue and eigenvector of a matrix. For details see, for example, [@Golub.Van-Loan.1996].
...
due to a fascinating fact known as the *circular law* [@Ginibre.1965].
The relationship between the eigenvalues (and a related object called singular values) of random matrices has been shown to have deep connections to proper initialization of neural networks as was discussed in @Pennington.Schoenholz.Ganguli.2017 and subsequent works.
