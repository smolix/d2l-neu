# Geometry and Linear Algebraic Operations {#sec-geometry-linear-algebraic-ops}

In @sec-linear-algebra, we encountered the basics of linear algebra
...
While @sec-linear-algebra contained enough machinery
...
As we have described in @sec-linear-algebra,
...
to a fixed reference called the *origin*.  This can be seen in @fig-grid.

![An illustration of visualizing vectors as points in the plane.  The first component of the vector gives the $\mathit{x}$-coordinate, the second component gives the $\mathit{y}$-coordinate.  Higher dimensions are analogous, although much harder to visualize.](../img/grid-points.svg){#fig-grid}
...
In this way, we consider all the vectors in figure @fig-arrow the same.

![Any vector can be visualized as an arrow in the plane.  In this case, every vector drawn is a representation of the vector $(3,2)^\top$.](../img/par-vec.svg){#fig-arrow}
...
and then follow the directions given by the other, as is seen in @fig-add-vec.

![We can visualize vector addition by first following one vector, and then another.](../img/vec-add.svg){#fig-add-vec}
...
As we saw in @sec-linear-algebra,
if we take two column vectors $\mathbf{u}$ and $\mathbf{v}$,
we can form their dot product by computing:

$$\mathbf{u}^\top\mathbf{v} = \sum_i u_i\cdot v_i.$$ {#eq-dot-def}

Because @eq-dot-def is symmetric, we will mirror the notation
...
The dot product @eq-dot-def also admits a geometric interpretation: it is closely related to the angle between two vectors.  Consider the angle shown in @fig-angle.

![Between any two vectors in the plane there is a well defined angle $\theta$.  We will see this angle is intimately tied to the dot product.](../img/vec-angle.svg){#fig-angle}
...
$$\theta = \arccos\left(\frac{\mathbf{v}\cdot\mathbf{w}}{\|\mathbf{v}\|\|\mathbf{w}\|}\right).$$ {#eq-angle-forumla}
...
By recalling the connection between dot products and angles above @eq-angle-forumla,
we can see that this is equivalent to
$$
\|\mathbf{v}\|\|\mathbf{w}\|\cos(\theta) = 1 \; \iff \; \|\mathbf{v}\|\cos(\theta) = \frac{1}{\|\mathbf{w}\|} = \frac{1}{\sqrt{5}}.
$$

![Recalling trigonometry, we see the formula $\|\mathbf{v}\|\cos(\theta)$ is the length of the projection of the vector $\mathbf{v}$ onto the direction of $\mathbf{w}$](../img/proj-vec.svg){#fig-vector-project}
...
onto the direction of $\mathbf{w}$ is exactly $1/\|\mathbf{w}\|$, as is shown in @fig-vector-project.
...
and the other side above as we see in @fig-space-division.

![If we now consider the inequality version of the expression, we see that our hyperplane (in this case: just a line) separates the space into two halves.](../img/space-division.svg){#fig-space-division}
...
The two inequalities again define the two sides of the plane as is shown in @fig-higher-division.

![Hyperplanes in any dimension separate the space into two halves.](../img/space-division-3d.svg){#fig-higher-division}
...
Through @sec-linear-algebra and the above discussions,
...
but the grid structure must remain as you see in @fig-grid-transform.

![The matrix $\mathbf{A}$ acting on the given basis vectors.  Notice how the entire grid is transported along with it.](../img/grid-transform.svg){#fig-grid-transform}
...
Consider the grid image from before, but now with a highlighted region (@fig-grid-filled).

![The matrix $\mathbf{A}$ again distorting the grid.  This time, I want to draw particular attention to what happens to the highlighted square.](../img/grid-transform-filled.svg){#fig-grid-filled}
...
In @sec-linear-algebra the concept of tensors was introduced.
...
$$ c_{i, j} = \sum_{k} a_{i, k}b_{k, j}.$$ {#eq-geometry-linear-algebraic-ops-auto-1}
