...
Before we get started, let's outline the relationship between machine learning and information theory. Machine learning aims to extract interesting signals from data and make critical predictions.  On the other hand, information theory studies encoding, decoding, transmitting, and manipulating information. As a result, information theory provides fundamental language for discussing the information processing in machine learned systems. For example, many machine learning applications use the cross-entropy loss as described in @sec-softmax.  This loss can be directly derived from information theoretic considerations.
...
$$I(X) = - \log_2 (p),$$ {#eq-information-theory-auto-1}
...
$$I(\textrm{"0010"}) = - \log (p(\textrm{"0010"})) = - \log \left( \frac{1}{2^4} \right) = 4 \textrm{ bits}.$$ {#eq-information-theory-auto-2}
...
$$H(X) = - E_{x \sim P} [\log p(x)].$$ {#eq-ent-def}
...
$$H(X) = - \int_x p(x) \log p(x) \; dx.$$ {#eq-information-theory-auto-3}
...
You may be curious: in the entropy definition @eq-ent-def, why do we use an expectation of a negative logarithm? Here are some intuitions.
...
$$H(S) = \sum_i {p_i \cdot I(s_i)} = - \sum_i {p_i \cdot \log p_i}.$$ {#eq-information-theory-auto-4}


### Properties of Entropy

By the above examples and interpretations, we can derive the following properties of entropy @eq-ent-def. Here, we refer to $X$ as an event and $P$ as the probability distribution of $X$.
...
Similar to entropy of a single random variable @eq-ent-def, we define the *joint entropy* $H(X, Y)$ of a pair random variables $(X, Y)$ as

$$H(X, Y) = -E_{(x, y) \sim P} [\log p_{X, Y}(x, y)]. $$ {#eq-joint-ent-def}
...
$$ H(Y \mid X) = - E_{(x, y) \sim P} [\log p(y \mid x)],$$ {#eq-cond-ent-def}
...
$$H(Y \mid X) = - \int_x \int_y p(x, y) \ \log p(y \mid x) \;dx \;dy.$$ {#eq-information-theory-auto-6}
...
$$H(Y \mid X) = H(X, Y) - H(X).$$ {#eq-information-theory-auto-7}
...
Now, let's implement conditional entropy @eq-cond-ent-def from scratch.
...
$$I(X, Y) = E_{x} E_{y} \left\{ p_{X, Y}(x, y) \log\frac{p_{X, Y}(x, y)}{p_X(x) p_Y(y)} \right\}. $$ {#eq-mut-ent-def}


We can summarize all of these relationships in image @fig-mutual-information.  It is an excellent test of intuition to see why the following statements are all also equivalent to $I(X, Y)$.

* $H(X) - H(X \mid Y)$
* $H(Y) - H(Y \mid X)$
* $H(X) + H(Y) - H(X, Y)$

![Mutual information's relationship with joint entropy and conditional entropy.](../img/mutual-information.svg){#fig-mutual-information}


In many ways we can think of the mutual information @eq-mut-ent-def as principled extension of correlation coefficient we saw in @sec-random-variables.  This allows us to ask not only for linear relationships between variables, but for the maximum information shared between the two random variables of any kind.
...
Rather than memorizing the definition of mutual information @eq-mut-ent-def, you only need to keep in mind its notable properties:
...
$$D_{\textrm{KL}}(P\|Q) = E_{x \sim P} \left[ \log \frac{p(x)}{q(x)} \right].$$ {#eq-kl-def}

As with the pointwise mutual information @eq-pmi-def, we can again provide an interpretation of the logarithmic term:  $-\log \frac{q(x)}{p(x)} = -\log(q(x)) - (-\log(p(x)))$ will be large and positive if we see $x$ far more often under $P$ than we would expect for $Q$, and large and negative if we see the outcome far less than expected.  In this way, we can interpret it as our *relative* surprise at observing the outcome compared to how surprised we would be observing it from our reference distribution.
...
Let's take a look at some properties of the KL divergence @eq-kl-def.
...
* There is a close relationship between KL divergence and mutual information. Besides the relationship shown in @fig-mutual-information, $I(X, Y)$ is also numerically equivalent with the following terms:
...
$$\textrm{CE}(P, Q) = - E_{x \sim P} [\log(q(x))].$$ {#eq-ce-def}
...
As alluded in the beginning of this section, cross-entropy @eq-ce-def can be used to define a loss function in the optimization problem. It turns out that the following are equivalent:
...
$$ y_{ij} = \begin{cases}1 & j \in J; \\ 0 &\textrm{otherwise.}\end{cases}$$ {#eq-information-theory-auto-8}
...
$$\mathbf{p}^\mathbf{z} = \prod_{j=1}^k p_{j}^{z_{j}}.$$ {#eq-information-theory-auto-9}
