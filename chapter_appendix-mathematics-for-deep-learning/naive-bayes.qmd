...
MNIST [@LeCun.Bottou.Bengio.ea.1998] is one of widely used datasets. It contains 60,000 images for training and 10,000 images for validation. Each image contains a handwritten digit from 0 to 9. The task is classifying each image into the corresponding digit.
...
In a classification task, we map an example into a category. Here an example is a grayscale $28\times 28$ image, and a category is a digit. (Refer to @sec-softmax for a more detailed explanation.)
...
$$\hat{y} = \mathrm{argmax} \> p(y  \mid  \mathbf{x}).$$ {#eq-naive-bayes-auto-1}
...
$$\hat{y} = \mathrm{argmax}_y \> p(y  \mid  \mathbf{x}) = \mathrm{argmax}_y \> \frac{p( \mathbf{x}  \mid  y) p(y)}{p(\mathbf{x})}.$$ {#eq-naive-bayes-auto-2}
...
$$p(x_1  \mid y) \cdot p(x_2  \mid  x_1, y) \cdot ... \cdot p( x_d  \mid  x_1, ..., x_{d-1}, y).$$ {#eq-naive-bayes-auto-3}
...
$$\hat{y} = \mathrm{argmax}_y \> \prod_{i=1}^d p(x_i  \mid  y) p(y).$$ {#eq-naive-bayes-auto-4}
...
$$\begin{aligned}\hat{y} &= \mathrm{argmax}_ y \ p(y)\prod_{i=1}^d   p(x_t = t_i \mid y) \\ &= \mathrm{argmax}_y \ P_y[y]\prod_{i=1}^d \ P_{xy}[i, y]^{t_i}\, \left(1 - P_{xy}[i, y]\right)^{1-t_i}\end{aligned}$$ {#eq-naive-bayes-estimation}
...
Now we can use @eq-naive-bayes-estimation to predict a new image. Given $\mathbf x$, the following functions computes $p(\mathbf x \mid y)p(y)$ for every $y$.
...
Since the logarithm is an increasing function, we can rewrite @eq-naive-bayes-estimation as
