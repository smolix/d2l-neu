...
$$P(\textrm{distance is in an}\; \epsilon \textrm{-sized interval around}\; x ) \approx \epsilon \cdot p(x).$$ {#eq-pdf-deriv}

Indeed, @eq-pdf-deriv precisely defines the *probability density function*.  It is a function $p(x)$ which encodes the relative probability of hitting near one point vs. another.  Let's visualize what such a function might look like.
...
$$P(X \; \textrm{is in an}\; \epsilon \textrm{-sized interval around}\; x ) \approx \epsilon \cdot p(x).$$ {#eq-pdf-def}
...
Second, let's imagine that we slice up the $\mathbb{R}$ into an infinite number of slices which are $\epsilon$ wide, say with slices $(\epsilon\cdot i, \epsilon \cdot (i+1)]$.  For each of these, we know from @eq-pdf-def the probability is approximately
...
This is nothing more than the approximation of an integral discussed in @sec-integral-calculus, thus we can say that
...
$$\int_{-\infty}^{\infty} p(x) \; dx = 1.$$ {#eq-pdf-int-one}

We interpret this function by using integration to obtain the probability our random variable is in a specific interval:

$$P(X\in(a, b]) = \int _ {a}^{b} p(x) \; dx.$$ {#eq-pdf-int-int}

In @sec-distributions we will see a number of common distributions, but let's continue working in the abstract.
...
In particular, by using @eq-pdf-int-int, we define the c.d.f. for a random variable $X$ with density $p(x)$ by
...
$$\mu_X = E[X] = \sum_i x_i p_i.$$ {#eq-exp-def}
...
As a minimalistic example that we will examine throughout this section, let's take $X$ to be the random variable which takes the value $a-2$ with probability $p$, $a+2$ with probability $p$ and $a$ with probability $1-2p$.  We can compute using @eq-exp-def that, for any possible choice of $a$ and $p$, the mean is
...
$$\sigma_X^2 = \textrm{Var}(X) = E\left[(X-\mu_X)^2\right] = E[X^2] - \mu_X^2.$$ {#eq-var-def}

The last equality in @eq-var-def holds by expanding out the definition in the middle, and applying the properties of expectation.
...
Thus, we see that by @eq-var-def our variance is
...
$$P\left(X \not\in [\mu_X - \alpha\sigma_X, \mu_X + \alpha\sigma_X]\right) \le \frac{1}{\alpha^2}.$$ {#eq-chebyshev}
...
Similarly, using @eq-var-def the variance can be written as
...
To see what goes astray, let's try to compute the variance of this.  This would involve using @eq-var-def computing
...
However, looking deeper shows an even more disturbing result.  Let's try to compute the mean using @eq-exp-def.  Using the change of variables formula, we see
...
The above work all assumes we are working with a single real valued random variable.  But what if we are dealing with two or more potentially highly correlated random variables?  This circumstance is the norm in machine learning: imagine random variables like $R_{i, j}$ which encode the red value of the pixel at the $(i, j)$ coordinate in an image, or $P_t$ which is a random variable given by a stock price at time $t$.  Nearby pixels tend to have similar color, and nearby times tend to have similar prices.  We cannot treat them as separate random variables, and expect to create a successful model (we will see in @sec-naive-bayes a model that under-performs due to such an assumption).  We need to develop the mathematical language to handle these correlated continuous random variables.

Thankfully, with the multiple integrals in @sec-integral-calculus we can develop such a language.  Suppose that we have, for simplicity, two random variables $X, Y$ which can be correlated.  Then, similar to the case of a single variable, we can ask the question:
...
![By summing along the columns of our array of probabilities, we are able to obtain the marginal distribution for just the random variable represented along the $\mathit{x}$-axis.](../img/marginal.svg){#fig-marginal}

This tells us to add up the value of the density along a series of squares in a line as is shown in @fig-marginal.  Indeed, after canceling one factor of epsilon from both sides, and recognizing the sum on the right is the integral over $y$, we can conclude that
...
$$\sigma_{XY} = \textrm{Cov}(X, Y) = \sum_{i, j} (x_i - \mu_X) (y_j-\mu_Y) p_{ij}. = E[XY] - E[X]E[Y].$$ {#eq-cov-def}
...
For continuous random variables, much the same story holds.  At this point, we are pretty comfortable with doing the transition between discrete and continuous, so we will provide the continuous analogue of @eq-cov-def without any derivation.
...
$$\rho(X, Y) = \frac{\textrm{Cov}(X, Y)}{\sigma_{X}\sigma_{Y}},$$ {#eq-cor-def}
...
Returning to our explicit discrete example above, we can see that $\sigma_X = 1$ and $\sigma_Y = 2$, so we can compute the correlation between the two random variables using @eq-cor-def to see that
...
$$\sigma_{Y} = \sigma_{aX+b} = |a|\sigma_{X},$$ {#eq-random-variables-auto-1}

$$\textrm{Cov}(X, Y) = \textrm{Cov}(X, aX+b) = a\textrm{Cov}(X, X) = a\textrm{Var}(X),$$ {#eq-random-variables-auto-2}

and thus by @eq-cor-def that
