# Using AWS EC2 Instances {#sec-aws}

In this section, we will show you how to install all libraries on a raw Linux machine. Recall that in @sec-sagemaker we discussed how to use Amazon SageMaker, while building an instance by yourself costs less on AWS. The walkthrough includes three steps:
...
This process applies to other instances (and other clouds), too, albeit with some minor modifications. Before going forward, you need to create an AWS account, see @sec-sagemaker for more details.


## Creating and Running an EC2 Instance

After logging into your AWS account, click "EC2" (@fig-aws) to go to the EC2 panel.

![Open the EC2 console.](../img/aws.png){#fig-aws width="400px"}

@fig-ec2 shows the EC2 panel.

![The EC2 panel.](../img/ec2.png){#fig-ec2 width="700px"}

### Presetting Location
Select a nearby data center to reduce latency, e.g., "Oregon" (marked by the red box in the top-right of @fig-ec2). If you are located in China,
...
@fig-ec2. 
@fig-limits shows an example of such a
...
![Instance quantity restrictions.](../img/limits.png){#fig-limits width="700px"}


### Launching an Instance

Next, click the "Launch Instance" button marked by the red box in @fig-ec2 to launch your instance.

We begin by selecting a suitable Amazon Machine Image (AMI). Select an Ubuntu instance (@fig-ubuntu).


![Choose an AMI.](../img/ubuntu-new.png){#fig-ubuntu width="700px"}

EC2 provides many different instance configurations to choose from. This can sometimes feel overwhelming to a beginner. @tbl-ec2 lists different suitable machines.
...
the instance. If you do not have a key pair, click "Create new key pair" in @fig-keypair to generate a key pair. Subsequently,
you can select the
...
![Select a key pair.](../img/keypair.png){#fig-keypair width="500px"}

In this example, we will keep the default configurations for "Network settings" (click the "Edit" button to configure items such as the subnet and security groups). We just increase the default hard disk size to 64 GB (@fig-disk). Note that CUDA by itself already takes up 4 GB.

![Modify the hard disk size.](../img/disk.png){#fig-disk width="700px"}


Click "Launch Instance" to launch the created
instance. Click the
instance ID shown in @fig-launching to view the status of this instance.

![Click the instance ID.](../img/launching.png){#fig-launching width="700px"}

### Connecting to the Instance

As shown in @fig-connect, after the instance state turns green, right-click the instance and select `Connect` to view the instance access method.

![View the instance access method.](../img/connect.png){#fig-connect width="700px"}
...
![View instance access and startup method.](../img/chmod.png){#fig-chmod width="400px"}


Now, copy the SSH command in the lower red box of @fig-chmod and paste onto the command line:
...
Here we download CUDA 12.1. Visit NVIDIA's [official repository](https://developer.nvidia.com/cuda-toolkit-archive) to find the download link as shown in @fig-cuda.

![Find the CUDA 12.1 download address.](../img/cuda121.png){#fig-cuda width="500px"}
...
just follow steps in @sec-chap-installation
...
@fig-jupyter shows the possible output after you run the Jupyter Notebook. The last row is the URL for port 8888.

![Output after running the Jupyter Notebook. The last row is the URL for port 8888.](../img/jupyter.png){#fig-jupyter width="700px"}

Since you used port forwarding to port 8889,
copy the last row in the red box of @fig-jupyter,
...
right-click on the example in @fig-connect and select "Image" $\rightarrow$
...
@fig-ubuntu, you must use the "My AMIs" option on the left to select your saved
