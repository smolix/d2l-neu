# Large-Scale Pretraining with Transformers {#sec-large-pretraining-transformers}
...
with English--French pairs (@sec-transformer)
...
(@sec-environment-and-distribution-shift).
...
training tokens, and training compute [@kaplan2020scaling].
...
(discussed in @sec-vision-transformer).
More recent success stories include Gato, a *generalist* model
that can play Atari, caption images, chat, and act as a robot [@reed2022generalist]. Gato is a single  Transformer that scales well when pretrained on diverse modalities,
...
which can be processed akin to text tokens (@sec-transformer)
or image patches (@sec-vision-transformer) by Transformers.
...
the Transformer architecture in @fig-transformer
...
For example, vision Transformers depicted in @fig-vit
...
As illustrated in @fig-bert-encoder-only,
...
(right of @fig-bert-encoder-only)
...
![Fine-tuning BERT for sentiment analysis.](../img/bert-finetune-classification.svg){#fig-bert-finetune-classification}

@fig-bert-finetune-classification illustrates
...
a BERT variant of the same size, on 2000 billion tokens [@Liu.Ott.Goyal.ea.2019].
Other derivatives of BERT improved model architectures or pretraining objectives,
such as ALBERT (enforcing parameter sharing) [@lan2019albert],
SpanBERT (representing and predicting spans of text) [@joshi2020spanbert],
DistilBERT (lightweight via knowledge distillation) [@sanh2019distilbert],
and ELECTRA (replaced token detection) [@clark2019electra].
Moreover, BERT inspired Transformer pretraining in computer vision,
such as with vision Transformers [@Dosovitskiy.Beyer.Kolesnikov.ea.2021],
Swin Transformers [@liu2021swin],
and MAE (masked autoencoders) [@he2022masked].
...
(multi-head attention of decoder in @fig-transformer)
...
pattern (masked multi-head attention of decoder in @fig-transformer),
...
BART [@lewis2019bart] and T5 [@raffel2020exploring]
...
Consider the example in @fig-t5-encoder-decoder,
...
As shown in @fig-t5-encoder-decoder,
...
from the web [@raffel2020exploring].
...
![Fine-tuning T5 for text summarization. Both the task description and article tokens are fed into the Transformer encoder for predicting the summary.](../img/t5-finetune-summarization.svg){#fig-t5-finetune-summarization}

@fig-t5-finetune-summarization
...
for better computational efficiency [@fedus2022switch].
In a text-to-image model called Imagen,
text is input to a frozen T5 encoder (T5-XXL)
with 4.6 billion parameters [@saharia2022photorealistic].
The photorealistic text-to-image examples in @fig-imagen
suggest that the T5 encoder alone may effectively
represent text even without fine-tuning.

![Text-to-image examples by the Imagen model, whose text encoder is from T5 (figures taken from @saharia2022photorealistic).](../img/imagen.png){#fig-imagen width="700px"}
...
depicted in @fig-transformer.
Nowadays, decoder-only Transformers have been the *de facto* architecture
in large-scale language modeling (@sec-language-model),
...
GPT-2, was introduced one year later [@Radford.Wu.Child.ea.2019].
Compared with the original Transformer decoder in GPT, pre-normalization
(discussed in @sec-vit-encoder)
...
![Zero-shot, one-shot, few-shot in-context learning with language models (Transformer decoders). No parameter update is needed.](../img/gpt-3-xshot.svg){#fig-gpt-3-xshot}

Before explaining the more computationally efficient use
of language models without parameter update,
recall @sec-rnn-scratch that a language model
...
This learning paradigm is called *in-context learning* [@brown2020language],
which can be further categorized
into *zero-shot*, *one-shot*, and *few-shot*,
when there is no, one, and a few task-specific input--output examples (@fig-gpt-3-xshot).


![Aggregate performance of GPT-3 for all 42 accuracy-denominated benchmarks (caption adapted and figure taken from @brown2020language).](../img/gpt3-xshot-scaling.png){#fig-gpt3-xshot-scaling width="400px"}

These three settings were tested in GPT-3 [@brown2020language],
...
(at the right in @fig-gpt-decoder-only)
are sparser at alternating layers.
Pretrained with 300 billion tokens,
GPT-3 performs better with larger model size,
where few-shot performance increases most rapidly (@fig-gpt3-xshot-scaling).

The subsequent GPT-4 model did not fully disclose technical details in its report [@openai2023gpt4].
...
@fig-gpt3-xshot-scaling empirically demonstrates scalability
...
in training larger Transformers with more data and compute [@kaplan2020scaling].

![Transformer language model performance improves smoothly as we increase the model size, dataset size, and amount of compute used for training. For optimal performance all three factors must be scaled up in tandem. Empirical performance has a power-law relationship with each individual factor when not bottlenecked by the other two (caption adapted and figure taken from @kaplan2020scaling).](../img/scaling-power-law.png){#fig-scaling-power-law3 width="700px"}

As shown in @fig-scaling-power-law3,
...
still remains a matter of debate [@hoffmann2022training].

![Transformer language model training runs (figure taken from @kaplan2020scaling).](../img/scaling-sample-conv.png){#fig-scaling-sample-conv width="700px"}

As well as increased performance, large models also enjoy better sample efficiency than small models. @fig-scaling-sample-conv shows that large models need fewer training samples (tokens processed) to perform at the same level achieved by small models, and performance is scaled smoothly with compute.


![GPT-3 performance (cross-entropy validation loss) follows a power-law trend with the amount of compute used for training. The power-law behavior observed in @kaplan2020scaling continues for an additional two orders of magnitude with only small deviations from the predicted curve. Embedding parameters are excluded from compute and parameter counts (caption adapted and figure taken from @brown2020language).](../img/scaling-gpt3.png){#fig-scaling-gpt3 width="250px"}


The empirical scaling behaviors in @kaplan2020scaling have been tested in subsequent large Transformer models. For example, GPT-3 supported this hypothesis with two more orders of magnitude in @fig-scaling-gpt3.


## Large Language Models

The scalability of Transformers in the GPT series has inspired subsequent large language models. 
The GPT-2 Transformer decoder was used for training the 530-billion-parameter Megatron-Turing NLG [@smith2022using] with 270 billion training tokens. Following the GPT-2 design, the 280-billion-parameter Gopher [@rae2021scaling] pretrained with 300 billion tokens, performed competitively across diverse tasks. 
Inheriting the same architecture and using the same compute budget of Gopher, Chinchilla [@hoffmann2022training] is a substantially smaller (70 billion parameters) model that trains for much longer (1.4 trillion training tokens), outperforming Gopher on many tasks and with more emphasis on the number of tokens than on the number of parameters.
To continue the scaling line of language modeling, 
PaLM (Pathway Language Model) [@chowdhery2022palm], a 540-billion-parameter Transformer decoder with modified designs pretrained on 780 billion tokens, outperformed average human performance on the BIG-Bench benchmark [@srivastava2022beyond]. Its later version, PaLM 2 [@anil2023palm], scaled data and model roughly 1:1 and improved multilingual and reasoning capabilities. 
Other large language models, such as Minerva  [@lewkowycz2022solving] that further trains a generalist (PaLM) and Galactica [@taylor2022galactica] that is not trained on a general corpus, have shown promising quantitative and scientific reasoning capabilities.


Open-sourced releases, such as OPT (Open Pretrained Transformers) [@zhang2022opt], BLOOM [@scao2022bloom], and FALCON [@penedo2023refinedweb],
democratized research and use of large language models.
Focusing on computational efficiency at inference time,
the open-sourced Llama 1 [@touvron2023llama] outperformed much larger models by training on more tokens than had been typically used. The updated Llama 2 [@touvron2023llama2] further increased the pretraining corpus by 40%, leading to product models that may match the performance of competitive close-sourced models. 


@wei2022emergent discussed emergent abilities of large language models that are present in larger models, but not in smaller models.
However, simply increasing model size does not inherently make models follow human instructions better.
@wei2021finetuned; @sanh2021multitask have found that fine-tuning large language models
...
tasks zero-shot [@qin2023chatgpt].
@bai2022constitutional replaced human inputs (e.g., human-labeled data) with model outputs
...
Sampling multiple reasoning paths [@wang2023self], diversifying few-shot demonstrations [@zhang2023automatic], 
and reducing complex problems to sub-problems [@zhou2023least]
...
chain-of-thought reasoning with decent accuracy [@kojima2022large].
Even for multimodal inputs consisting of both text and images,
language models can perform multimodal chain-of-thought reasoning with higher accuracy than using text input only [@zhang2023multicot].
...
(i) Chinchilla [@hoffmann2022training] was further extended to Flamingo [@alayrac2022flamingo], a visual language model for few-shot learning;
(ii) GPT-2 [@Radford.Wu.Child.ea.2019] and the vision Transformer encode text and images in CLIP (Contrastive Language-Image Pre-training) [@radford2021learning], whose image and text embeddings were later adopted in the DALL-E 2 text-to-image system [@ramesh2022hierarchical]. Although there have been no systematic studies on Transformer scalability in multimodal pretraining yet, an all-Transformer text-to-image model called Parti [@yu2022scaling] shows potential of scalability across modalities:
a larger Parti is more capable of high-fidelity image generation and content-rich text understanding (@fig-parti).


![Image examples generated from the same text by the Parti model of increasing sizes (350M, 750M, 3B, 20B) (examples taken from @yu2022scaling).](../img/parti.png){#fig-parti width="700px"}
