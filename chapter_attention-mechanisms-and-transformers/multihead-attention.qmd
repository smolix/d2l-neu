...
# Multi-Head Attention {#sec-multihead-attention}
...
![Multi-head attention, where multiple heads are concatenated then linearly transformed.](../img/multi-head-attention.svg){#fig-multi-head-attention}
...
$$\mathbf{h}_i = f(\mathbf W_i^{(q)}\mathbf q, \mathbf W_i^{(k)}\mathbf k,\mathbf W_i^{(v)}\mathbf v) \in \mathbb R^{p_v},$$ {#eq-multihead-attention-auto-1}
...
in @sec-attention-scoring-functions.
...
$$\mathbf W_o \begin{bmatrix}\mathbf h_1\\\vdots\\\mathbf h_h\end{bmatrix} \in \mathbb{R}^{p_o}.$$ {#eq-multihead-attention-auto-2}
