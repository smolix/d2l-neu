...
# Self-Attention and Positional Encoding {#sec-self-attention-and-positional-encoding}
...
such architectures are typically described as *self-attention* models [@Lin.Feng.Santos.ea.2017; @Vaswani.Shazeer.Parmar.ea.2017], 
and elsewhere described as *intra-attention* model [@Cheng.Dong.Lapata.2016; @Parikh.Tackstrom.Das.ea.2016; @Paulus.Xiong.Socher.2017].
...
$$\mathbf{y}_i = f(\mathbf{x}_i, (\mathbf{x}_1, \mathbf{x}_1), \ldots, (\mathbf{x}_n, \mathbf{x}_n)) \in \mathbb{R}^d$$ {#eq-self-attention-and-positional-encoding-auto-1}

according to the definition of attention pooling in
@eq-attention-pooling.
...
## Comparing CNNs, RNNs, and Self-Attention {#sec-cnn-rnn-self-attention}
...
within the sequence [@Hochreiter.Bengio.Frasconi.ea.2001].


![Comparing CNN (padding tokens are omitted), RNN, and self-attention architectures.](../img/cnn-rnn-self-attention.svg){#fig-cnn-rnn-self-attention}
...
As @fig-cnn-rnn-self-attention shows,
...
with kernel size 3 in @fig-cnn-rnn-self-attention.
...
is $\mathcal{O}(nd^2)$.
According to @fig-cnn-rnn-self-attention,
...
As we can see from @fig-cnn-rnn-self-attention,
...
$$\begin{aligned} p_{i, 2j} &= \sin\left(\frac{i}{10000^{2j/d}}\right),\\p_{i, 2j+1} &= \cos\left(\frac{i}{10000^{2j/d}}\right).\end{aligned}$$ {#eq-positional-encoding-def}
...
1. Can we assign different learned embeddings according to different offsets between queries and keys that are compared in self-attention? Hint: you may refer to relative position embeddings [@shaw2018self; @huang2018music].
