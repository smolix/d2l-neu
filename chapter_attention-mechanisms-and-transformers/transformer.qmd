...
# The Transformer Architecture {#sec-transformer}


We have compared CNNs, RNNs, and self-attention in
@sec-cnn-rnn-self-attention.
...
that still rely on RNNs for input representations [@Cheng.Dong.Lapata.2016; @Lin.Feng.Santos.ea.2017; @Paulus.Xiong.Socher.2017],
the Transformer model
is solely based on attention mechanisms
without any convolutional or recurrent layer [@Vaswani.Shazeer.Parmar.ea.2017].
...
![The Transformer architecture.](../img/transformer.svg){#fig-transformer width="320px"}


Now we provide an overview of the
Transformer architecture in @fig-transformer.
At a high level,
...
Inspired by the ResNet design of @sec-resnet,
...
followed by layer normalization [@Ba.Kiros.Hinton.2016].
...
in @sec-multihead-attention
and positional encoding in @sec-positional-encoding.
In the following, we will implement
the rest of the Transformer model.

## Positionwise Feed-Forward Networks {#sec-positionwise-ffn}
...
Now let's focus on the "add & norm" component in @fig-transformer.
...
a minibatch.
As discussed in @sec-layer-normalization-in-bn,
...
## Encoder {#sec-transformer-encoder}
...
1. How would you improve the computational and memory efficiency of Transformers? Hint: you may refer to the survey paper by @Tay.Dehghani.Bahri.ea.2020 .
