...
# Transformers for Vision {#sec-vision-transformer}
...
in various natural language processing tasks [@Radford.Narasimhan.Salimans.ea.2018; @Radford.Wu.Child.ea.2019; @brown2020language; @Devlin.Chang.Lee.ea.2018; @raffel2020exploring].
However, in the field of computer vision
the dominant architecture has remained
the CNN (@sec-chap-modern-cnn).
...
Recently, @ramachandran2019stand proposed
...
Then, @cordonnier2020relationship theoretically proved
...
which will finally be transformed for classification [@Dosovitskiy.Beyer.Kolesnikov.ea.2021].
...
It works exactly the same way as the original Transformer encoder in @fig-transformer,
only differing in the position of normalization.
Since the “&lt;cls&gt;” token attends to all the image patches
via self-attention (see @fig-cnn-rnn-self-attention),
...
with patch embedding in @fig-vit.
...
## Vision Transformer Encoder {#sec-vit-encoder}
...
(see @sec-positionwise-ffn).
First, here the activation function uses the Gaussian error linear unit (GELU),
which can be considered as a smoother version of the ReLU [@Hendrycks.Gimpel.2016].
...
just follows the pre-normalization design in @fig-vit,
where normalization is applied right *before* multi-head attention or the MLP.
In contrast to post-normalization ("add & norm" in @fig-transformer),
where normalization is placed right *after* residual connections,
pre-normalization leads to more effective or efficient training for Transformers [@baevski2018adaptive; @wang2019learning; @xiong2020layer].
...
Just as in @sec-transformer-encoder,
...
Training a vision Transformer on the Fashion-MNIST dataset is just like how CNNs were trained in @sec-chap-modern-cnn.
...
does not outperform the ResNet in @sec-resnet.
...
such as translation invariance and locality (@sec-why-conv).
...
intrinsic superiority of Transformers in scalability [@Dosovitskiy.Beyer.Kolesnikov.ea.2021].
...
with data-efficient training strategies of DeiT [@touvron2021training].
However, the quadratic complexity of self-attention
(@sec-self-attention-and-positional-encoding)
...
with respect to image size (@sec-cnn-rnn-self-attention)
and reinstated convolution-like priors,
extending the applicability of Transformers to a range of computer vision tasks
beyond image classification with state-of-the-art results [@liu2021swin].
