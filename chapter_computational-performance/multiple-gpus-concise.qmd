# Concise Implementation for Multiple GPUs {#sec-multi-gpu-concise}
...
The mathematics and the algorithms are the same as in @sec-multi-gpu.
...
Let's use a slightly more meaningful network than LeNet from @sec-multi-gpu that is still sufficiently easy and quick to train.
We pick a ResNet-18 variant [@He.Zhang.Ren.ea.2016]. Since the input images are tiny we modify it slightly. In particular, the difference from @sec-resnet is that we use a smaller convolution kernel, stride, and padding at the beginning.
...
For a refresher on initialization methods see @sec-numerical-stability.

## MXNet

The `initialize` function allows us to initialize parameters on a device of our choice.
For a refresher on initialization methods see @sec-numerical-stability. What is particularly convenient is that it also allows us to initialize the network on *multiple* devices simultaneously. Let's try how this works in practice.
...
Next, let's replace the code to evaluate the accuracy by one that works in parallel across multiple devices. This serves as a replacement of the `evaluate_accuracy_gpu` function from @sec-lenet. The main difference is that we split a minibatch before invoking the network. All else is essentially identical.
