...
Note that full synchronization in fact requires two such passes — a *reduce-scatter* phase to build up the aggregated gradient (one shard landing on each node) followed by an *all-gather* phase that distributes each shard back to every node. Both phases have the same cost structure, which is why the leading factor of $2$ appears in the time estimate below.
...
If we use the same example of synchronizing 160 MB across 8 V100 GPUs we arrive at approximately $2 \cdot 160 \textrm{MB} / (3 \cdot 18 \textrm{GB/s}) \approx 6 \textrm{ms}$. Here the factor of $3$ in the denominator reflects the fact that several aggregation paths can run simultaneously: decomposing the NVLink network into two rings (see :numref:`fig_nvlink_twoloop`) yields one ring with double NVLink bandwidth plus one with regular bandwidth, giving an aggregate of roughly $3$ NVLink lanes of effective bandwidth per direction. This is better than using the PCIe bus, even though we are now using 8 GPUs. Note that in practice these numbers are a bit worse, since deep learning frameworks often fail to assemble communication into large burst transfers.
