...
to another image, i.e., *style transfer* [@Gatys.Ecker.Bethge.2016].
...
For example,
the content image in @fig-style-transfer is a landscape photo taken by us
...
![Given content and style images, style transfer outputs a synthesized image.](../img/style-transfer.svg){#fig-style-transfer}

## Method

@fig-style-transfer-model illustrates
...
Take @fig-style-transfer-model as an example.
...
![CNN-based style transfer process. Solid lines show the direction of forward propagation and dotted lines show backward propagation. ](../img/neural-style.svg){#fig-style-transfer-model}
...
We use the VGG-19 model pretrained on the ImageNet dataset to extract image features [@Gatys.Ecker.Bethge.2016].
...
$$\sum_{i, j} \left|x_{i, j} - x_{i+1, j}\right| + \left|x_{i, j} - x_{i, j+1}\right|$$ {#eq-neural-style-auto-1}
...
1. Can we apply style transfer for text? Hint: you may refer to the survey paper by @10.1145/3544903.3544906.
