# Single Shot Multibox Detection {#sec-ssd}

In @sec-bbox to @sec-object-detection-dataset,
...
@fig-ssd provides an overview of
...
in @sec-multiscale-object-detection.
Since
multiscale feature maps closer to the top of @fig-ssd
...
![As a multiscale object detection model, single-shot multibox detection mainly consists of a base network followed by several multiscale feature map blocks.](../img/ssd.svg){#fig-ssd}


In the following,
we will describe the implementation details
of different blocks in @fig-ssd. To begin with, we discuss how to implement
...
of the invoked `multibox_prior` function (described in @sec-anchor).
In the following,
...
described in @sec-object-detection-dataset.
...
described in @sec-normal-distribution-and-squared-loss.
Instead,
...
$$ - \alpha (1-p_j)^{\gamma} \log p_j.$$ {#eq-ssd-auto-1}
