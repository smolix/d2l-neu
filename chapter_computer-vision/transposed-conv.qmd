...
such as convolutional layers (@sec-conv-layer) and pooling layers (@sec-pooling),
...
As an example,
@fig-trans-conv illustrates
how transposed convolution with a $2\times 2$ kernel is computed for a $2\times 2$ input tensor.


![Transposed convolution with a $2\times 2$ kernel. The shaded portions are a portion of an intermediate tensor as well as the input and kernel tensor elements used for the  computation.](../img/trans_conv.svg){#fig-trans-conv}
...
In contrast to the regular convolution (in @sec-conv-layer) that *reduces* input elements
via the kernel,
...
We can construct the input tensor `X` and the kernel tensor `K` from @fig-trans-conv to validate the output of the above implementation of the basic two-dimensional transposed convolution operation.
...
in @fig-trans-conv-stride2.


![Transposed convolution with a $2\times 2$ kernel with stride of 2. The shaded portions are a portion of an intermediate tensor as well as the input and kernel tensor elements used for the  computation.](../img/trans_conv_stride2.svg){#fig-trans-conv-stride2}


The following code snippet can validate the transposed convolution output for stride of 2 in @fig-trans-conv-stride2.
...
## Connection to Matrix Transposition {#subsec-connection-to-mat-transposition}
...
1. In @subsec-connection-to-mat-transposition, the convolution input `X` and the transposed convolution output `Z` have the same shape. Do they have the same value? Why?
