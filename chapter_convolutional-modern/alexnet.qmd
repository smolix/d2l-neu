...
# Deep Convolutional Neural Networks (AlexNet) {#sec-alexnet}


Although CNNs were well known
in the computer vision and machine learning communities
following the introduction of LeNet [@LeCun.Jackel.Bottou.ea.1995],
...
and the watershed results of 2012 [@Krizhevsky.Sutskever.Hinton.2012],
neural networks were often surpassed by other machine learning methods,
such as kernel methods [@Scholkopf.Smola.2002], ensemble methods [@Freund.Schapire.ea.1996],
and structured estimation [@Taskar.Guestrin.Koller.2004].
...
consisted of manually engineering feature extraction pipelines, such as SIFT [@Lowe.2004], SURF [@Bay.Tuytelaars.Van-Gool.2006], and bags of visual words [@Sivic.Zisserman.2003].
Rather than *learning* the features, the features were *crafted*.
Most of the progress came from having more clever ideas for feature extraction on the one hand and deep insight into geometry [@Hartley.Zisserman.2000] on the other. The learning algorithm was often considered an afterthought.
...
including parameter initialization heuristics [@Glorot.Bengio.2010],
clever variants of stochastic gradient descent [@Kingma.Ba.2014],
non-squashing activation functions [@Nair.Hinton.2010],
and effective regularization techniques [@Srivastava.Hinton.Krizhevsky.ea.2014] were still missing.
...
1. Feed the data through a standard set of feature extractors such as the SIFT (scale-invariant feature transform) [@Lowe.2004], the SURF (speeded up robust features) [@Bay.Tuytelaars.Van-Gool.2006], or any number of other hand-tuned pipelines. OpenCV still provides SIFT extractors to this day!
...
Elegant theories proved the properties of various classifiers [@boucheron2005theory] and convex
optimization [@Boyd.Vandenberghe.2004] had become the mainstay for obtaining them.
...
is that features, geometry [@Hartley.Zisserman.2000; @hartley2009global], and engineering,
...
HOG (histograms of oriented gradient) [@Dalal.Triggs.2005],
...
by sparse coding [@olshausen1996emergence] remained an open challenge until the advent of modern CNNs.
It was not until @Dean.Corrado.Monga.ea.2012; @le2013building that the idea of generating features
from image data automatically gained significant traction.

The first modern CNN [@Krizhevsky.Sutskever.Hinton.2012], named
...
![Image filters learned by the first layer of AlexNet. Reproduction courtesy of @Krizhevsky.Sutskever.Hinton.2012.](../img/filters.png){#fig-filters width="400px"}
...
In 2009, the ImageNet dataset was released [@Deng.Dong.Socher.ea.2009],
...
were based on the most popular noun nodes in WordNet [@Miller.1995].
...
TinyImages dataset [@Torralba.Fergus.Freeman.2008], consisting of $32 \times 32$ pixel thumbnails.
...
[@schuhmann2022laion] contain billions of images with additional metadata.
...
for general computing operations [@Fernando.2004],
...
by a large margin [@Russakovsky.Deng.Huang.ea.2013].
...
![From LeNet (left) to AlexNet (right).](../img/alexnet.svg){#fig-alexnet}
...
See @Buslaev.Iglovikov.Khvedchenya.ea.2020 for an in-depth review of such preprocessing steps.
...
We construct a single-channel data example with both height and width of 224 to observe the output shape of each layer. It matches the AlexNet architecture in @fig-alexnet.
...
Although AlexNet was trained on ImageNet in @Krizhevsky.Sutskever.Hinton.2012,
...
Compared to LeNet in @sec-lenet,
...
Although it seems that there are only a few more lines in AlexNet's implementation than in LeNet's, it took the academic community many years to embrace this conceptual change and take advantage of its excellent experimental results. This was also due to the lack of efficient computational tools. At the time neither DistBelief [@Dean.Corrado.Monga.ea.2012] nor Caffe [@Jia.Shelhamer.Donahue.ea.2014] existed, and Theano [@Bergstra.Breuleux.Bastien.ea.2010] still lacked many distinguishing features. It was the availability of TensorFlow [@Abadi.Barham.Chen.ea.2016] that dramatically changed the situation.
