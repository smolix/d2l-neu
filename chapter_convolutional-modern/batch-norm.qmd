...
# Batch Normalization {#sec-batch-norm}
...
that consistently accelerates the convergence of deep networks [@Ioffe.Szegedy.2015].
Together with residual blocks---covered later in @sec-resnet --- batch normalization
...
Recall our application of MLPs to predicting house prices (@sec-kaggle-house).
Our first step when working with real data
was to standardize our input features to have
zero mean $\boldsymbol{\mu} = 0$ and unit variance $\boldsymbol{\Sigma} = \boldsymbol{1}$ across multiple observations [@friedman1987exploratory], frequently rescaling the latter so  that the diagonal is unity, i.e., $\Sigma_{ii} = 1$.
...
For a review of feature selection and extraction see the article of @guyon2008feature, for example.
Standardizing vectors also has the nice side-effect of constraining the function complexity of functions that act upon it. For instance, the celebrated radius-margin bound [@Vapnik95] in support vector machines and the Perceptron Convergence Theorem [@Novikoff62] rely on inputs of bounded norm. 
...
might not be beneficial. While this is not quite the reasoning that led to the invention of batch normalization [@Ioffe.Szegedy.2015], it is a useful way of understanding it and its cousin, layer normalization [@Ba.Kiros.Hinton.2016], within a unified framework.
...
such as AdaGrad [@Duchi.Hazan.Singer.2011], Adam [@Kingma.Ba.2014], Yogi [@Zaheer.Reddi.Sachan.ea.2018], or Distributed Shampoo [@anil2020scalable] aim to address this from the viewpoint of optimization, e.g., by adding aspects of second-order methods. 
...
inputs [@Bishop.1995]. It also forms the basis of dropout in @sec-dropout. As it turns out, quite serendipitously, batch normalization conveys all three benefits: preprocessing, numerical stability, and regularization.
...
$$\textrm{BN}(\mathbf{x}) = \boldsymbol{\gamma} \odot \frac{\mathbf{x} - \hat{\boldsymbol{\mu}}_{\mathcal{B}}}{\hat{\boldsymbol{\sigma}}_{\mathcal{B}}} + \boldsymbol{\beta}.$$ {#eq-batchnorm}
...
We calculate $\hat{\boldsymbol{\mu}}_{\mathcal{B}}$ and ${\hat{\boldsymbol{\sigma}}_{\mathcal{B}}}$ in @eq-batchnorm as follows:
...
@Teye.Azizpour.Smith.2018 and @Luo.Wang.Shao.ea.2018
...
In this form they closely resemble the behavior of dropout regularization of @sec-dropout,
...
@Ioffe.Szegedy.2015, in their original paper inserted batch normalization after the affine transformation
...
$$\mathbf{h} = \phi(\textrm{BN}(\mathbf{W}\mathbf{x} + \mathbf{b}) ).$$ {#eq-batch-norm-auto-1}
...
### Layer Normalization {#sec-layer-normalization-in-bn}
...
led @Ba.Kiros.Hinton.2016 to introduce the notion of *layer normalization*. It works just like
...
$$\mathbf{x} \rightarrow \textrm{LN}(\mathbf{x}) =  \frac{\mathbf{x} - \hat{\mu}}{\hat\sigma},$$ {#eq-batch-norm-auto-2}

where scaling and offset are applied coefficient-wise
and given by 

$$\hat{\mu} \stackrel{\textrm{def}}{=} \frac{1}{n} \sum_{i=1}^n x_i \textrm{ and }
\hat{\sigma}^2 \stackrel{\textrm{def}}{=} \frac{1}{n} \sum_{i=1}^n (x_i - \hat{\mu})^2 + \epsilon.$$ {#eq-batch-norm-auto-3}
...
below we apply it to a traditional LeNet model (@sec-lenet).
...
@sec-oo-design-training) as an attribute and during the model's forward pass,
...
The original paper proposing batch normalization [@Ioffe.Szegedy.2015], in addition to introducing a powerful and useful tool,
...
some [@Santurkar.Tsipras.Ilyas.ea.2018]
...
* For more robust models that are less sensitive to input perturbations, consider removing batch normalization [@wang2022removing].
