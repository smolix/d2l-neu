...
The inverted bottleneck outlived its mobile origins: ConvNeXt :cite:`liu2022convnet` adopted the same shape for its blocks, with the depthwise convolution enlarged to $7 \times 7$ and moved to the front. <!-- TODO(ch8): numref sec_convnext when 8.6 lands --> We will meet it again when we discuss design spaces in :numref:`sec_cnn-design`.
