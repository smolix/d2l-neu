...
# Convolutions for Images {#sec-conv-layer}
...
Based on our descriptions of convolutional layers in @sec-why-conv,
...
![Two-dimensional cross-correlation operation. The shaded portions are the first output element as well as the input and kernel tensor elements used for the output computation: $0\times0+1\times1+3\times2+4\times3=19$.](../img/correlation.svg){#fig-correlation}
...
$$(n_\textrm{h}-k_\textrm{h}+1) \times (n_\textrm{w}-k_\textrm{w}+1).$$ {#eq-conv-layer-auto-1}
...
Recall our observation from @sec-why-conv of the correspondence
...
as defined in @eq-2d-conv-discrete
...
To illustrate this, suppose that a convolutional layer performs *cross-correlation* and learns the kernel in @fig-correlation, which is here denoted as the matrix $\mathbf{K}$.
...
for the input in @fig-correlation
and $\mathbf{K}'$,
the same output in @fig-correlation
...
As described in @sec-why-conv-channels,
...
Let's continue to use @fig-correlation to explain the receptive field.
