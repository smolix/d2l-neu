# From Fully Connected Layers to Convolutions {#sec-why-conv}
...
(which itself has inspired many real-life imitations, such as that depicted in @fig-waldo).
...
are based on this approach [@Long.Shelhamer.Darrell.2015]. 
CNNs systematize this idea of *spatial invariance*,
exploiting it to learn useful representations
with fewer parameters.

![Can you find Waldo (image courtesy of William Murphy (Infomatique))?](../img/waldo-football.jpg){#fig-waldo width="400px"}
...
\sum_a \sum_b [\mathsf{V}]_{i, j, a, b}  [\mathbf{X}]_{i+a, j+b}.\end{aligned}$$ {#eq-why-conv-auto-1}
...
established above: translation invariance [@Zhang.ea.1988].
...
$$[\mathbf{H}]_{i, j} = u + \sum_a\sum_b [\mathbf{V}]_{a, b}  [\mathbf{X}]_{i+a, j+b}.$$ {#eq-why-conv-auto-2}
...
$$[\mathbf{H}]_{i, j} = u + \sum_{a = -\Delta}^{\Delta} \sum_{b = -\Delta}^{\Delta} [\mathbf{V}]_{a, b}  [\mathbf{X}]_{i+a, j+b}.$$ {#eq-conv-layer}

This reduces the number of parameters from $4 \times 10^6$ to $4 \Delta^2$, where $\Delta$ is typically smaller than $10$. As such, we reduced the number of parameters by another four orders of magnitude. Note that @eq-conv-layer, is what is called, in a nutshell, a *convolutional layer*. 
...
Let's briefly review why @eq-conv-layer is called a convolution. 
In mathematics, the *convolution* between two functions [@Rudin.1973],
...
$$(f * g)(i) = \sum_a f(a) g(i-a).$$ {#eq-why-conv-auto-3}

For two-dimensional tensors, we have a corresponding sum
with indices $(a, b)$ for $f$ and $(i-a, j-b)$ for $g$, respectively:

$$(f * g)(i, j) = \sum_a\sum_b f(a, b) g(i-a, j-b).$$ {#eq-2d-conv-discrete}

This looks similar to @eq-conv-layer, with one major difference.
...
@eq-conv-layer and @eq-2d-conv-discrete.
Our original definition in @eq-conv-layer more properly
describes a *cross-correlation*.
We will come back to this in the following section.


## Channels {#sec-why-conv-channels}

Returning to our Waldo detector, let's see what this looks like.
The convolutional layer picks windows of a given size
and weighs intensities according to the filter $\mathsf{V}$, as demonstrated in @fig-waldo-mask.
...
![Detect Waldo (image courtesy of William Murphy (Infomatique)).](../img/waldo-mask.jpg){#fig-waldo-mask width="400px"}
...
$$[\mathsf{H}]_{i,j,d} = \sum_{a = -\Delta}^{\Delta} \sum_{b = -\Delta}^{\Delta} \sum_c [\mathsf{V}]_{a, b, c, d} [\mathsf{X}]_{i+a, j+b, c},$$ {#eq-conv-layer-channels}
...
1. Assume that the size of the convolution kernel is $\Delta = 0$.
   ...
   architectures [@Lin.Chen.Yan.2013]. 
