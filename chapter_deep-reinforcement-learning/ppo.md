...
1. Run the normalized REINFORCE of :numref:`sec_baselines` with a deliberately oversized learning rate, $\alpha = 50$, on a handful of seeds. Many runs never learn at all: the first noisy updates saturate the softmax, exactly as the two-action example predicts, and the score vanishes. Now add an entropy bonus $-c \sum_a \pi_\theta(a \mid s) \log \pi_\theta(a \mid s)$ to the per-sample objective. Can any $c$ prevent the collapse without stalling the learning, and what does the bonus cost you when $\alpha = 2$?
...
Most of the eight unclipped seeds end dead, typically at return
$\approx 9$ — the saturation collapse, for real. *Which* seeds die
reshuffles from run to run; the rate does not — about four unclipped
runs in five never recover. Every clipped run reaches the ceiling, and
the clip touches under $2\%$ of samples:
