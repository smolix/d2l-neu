...
:numref:`sec_gan_objectives` closed by proposing to keep the log-loss game and change what the critic scores. This section carries that change out. The critic still assigns a scalar realness score to each individual sample, but the game now presents it with a pair, one real sample and one generated one, and pays it for ranking the real member higher. Two questions organize the section. What quantity does the pairing game evaluate at its optimal critic? The answer is a closed form, the Jensen--Shannon divergence between the two orderings of a real--fake pair, and its derivation is a short reduction to the game :numref:`sec_basic_gan` already solved. And what does pairing change for the generator? Its per-sample update weight turns from a threshold statistic into a rank statistic, the property behind this objective's place in R3GAN, the modern minimal recipe of :citet:`Huang.Gokaslan.Kuleshov.ea.2024` that :numref:`sec_gan_convergence` assembles and :numref:`sec_dcgan` trains on images. The section also states what pairing does not change: on disjoint supports the new value saturates exactly as the old one did.
...
Part (iii) depends on the logistic payoff: the pointwise optimum on the pair space is separable because the log ratio of two product measures is a sum of per-coordinate terms. Under other payoffs the Bayes-optimal pair critic is a nonlinear function of that log ratio, no longer a difference of single-sample scores, and the corresponding relativistic objective only bounds its lifted divergence from below. Exercise 5 works this out. With the lemma in hand, the value of the pairing game follows from results already proved.
...
The mutual-information reading of :numref:`sec_basic_gan` lifts along with the value. Draw $x \sim p$ and $x' \sim q$, present the two samples in a uniformly random order, and let the bit $b$ record which position holds the real one. The ordered pair $(a_1, a_2)$ is then a sample from the balanced mixture of $P$ and $Q$ with label $b$, and the identification of the divergence with a mutual information, applied on the pair space, gives

$$
d_{\mathrm{Rp}}(p, q) \;=\; I\big((a_1, a_2);\, b\big) \;=\; \log 2 - H\big(b \mid a_1, a_2\big).
$$
:eqlabel:`eq_gan_rp_mi`

The pairing divergence is the number of nats a randomly ordered pair carries about which of its members is real, exactly as $\mathrm{JS}(p, q)$ is the number of nats a single sample carries about its origin. In particular $0 \leq d_{\mathrm{Rp}} \leq \log 2$: the game is again playing for at most one bit.
...
The closed form permits an exact comparison between the pairing game and the single-sample game it lifted.
...
The classical weight measures a sample against a threshold: the critic's zero, a level the classical game pins and the pairing game leaves arbitrary. The relativistic weight is a rank statistic: the probability that $x'$ outranks a random real sample, invariant to the critic's additive constant exactly as the objective is. The distinction bears directly on mode collapse. A threshold can be satisfied wholesale, since a generator that places all of its mass on the far side of one decision boundary makes every per-sample factor look real, regardless of where the data distribution actually puts its mass, and collapse onto a few well-scored points is precisely such a configuration. A rank statistic offers no such shortcut, because improving it means improving the distribution of the generator's ranks against the whole real population. :citet:`Sun.Fang.Schwing.2020` convert this observation into a statement about the empirical loss landscape. For $n$ data points and $n$ generated points, the classical objective has at least $n^n - n!$ sub-optimal strict local minima: one for every way of assigning generated points to data points with at least one collision, which is to say every mode-dropping configuration. Under the relativistic objective, by contrast, every configuration can descend to a global minimum. We use their statement without reproducing the proof. These are finite-sample landscape results, and they neither contradict nor follow from part (c) of the proposition above. Pairing repairs where gradient descent can get stuck; it does not change what the objective is worth on separated supports.
...
The non-saturating swap is as old as the construction: :citet:`Jolicoeur-Martineau.2019` trains the generator this way from the start. The modern reference deserves a closer reading, because paper and code differ. The R3GAN paper presents the zero-sum game: the loss it displays, its Eq. (2), is a single minimax over $\Phi$, which places the generator on the saturating row of the table. The official implementation instead trains the generator on the non-saturating row, maximizing $E[\log \sigma(D(x') - D(x))]$ while the critic maximizes $\Phi$, so the implemented game is not zero-sum. The two variants do share their fixed point. :numref:`sec_basic_gan` verified this for the classical pair by evaluating the non-saturating loss at the optimal critic, and the same evaluation works here: at $D^\star = \lambda$ the swapped generator loss is $E[\log(1 + e^{\lambda(x) - \lambda(x')})]$, which Jensen's inequality bounds below by $\log(1 + e^{J})$ with $J = \mathrm{KL}(p \,\|\, q) + \mathrm{KL}(q \,\|\, p) \geq 0$, so the loss exceeds $\log 2$ whenever $q \neq p$ and attains it at $q = p$. Where the two variants differ is at the start of training, when generated samples rank badly. A reader who implements the paper's stated equation literally trains the weaker, saturating variant: a difference invisible in the equilibrium analysis and decisive in the first epochs.

One practical question remains before the experiment: how the expectation over $p \otimes q$ is estimated. A minibatch of $n$ real and $n$ generated samples supports two unbiased estimators of $\Phi$: pair the $i$-th real with the $i$-th generated sample, giving $n$ terms, or average over all $n^2$ cross pairs. The all-pairs average is the minimum-variance unbiased estimator, by the two-sample U-statistic theorem :cite:`Jolicoeur-Martineau.2020`, and it costs $O(n^2)$ against $O(n)$, the same trade the MMD estimator posed in :numref:`sec_gan_objectives`. The variance reduction did not translate into better generators in her experiments, which found the minimum-variance estimator no better and sometimes worse. The cheap diagonal estimator is therefore the standard choice, and the verification below needs neither: its sample space is small enough to evaluate $\Phi$ exactly.
...
- Additive shift: $\Phi(D + c) = \Phi(D)$ — the critic is identified only
...
::: {.slide title="What Pairing Buys, and What It Does Not"}
| property | statement |
|:---|:---|
