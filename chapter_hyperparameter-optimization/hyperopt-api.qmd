...
# Hyperparameter Optimization API {#sec-api-hpo}
...
libraries, such as Syne Tune [@salinas-automl22], Ray Tune
[@liaw-arxiv18] or Optuna [@akiba-sigkdd19].
...
as we did for random search in @sec-what-is-hpo. More sophisticated
...
from @sec-lenet. We being by defining the objective function, which
...
to be evaluated is the default setting used in @sec-lenet.
...
To illustrate this, we compare random search (see @sec-rs) and Bayesian
optimization [@snoek-nips12] on tuning the hyperparameters of a feed-forward
...
![Example any-time performance plot to compare two algorithms A and B.](../img/example_anytime_performance.svg){#example-anytime-performance}
...
1. The goal of this exercise is to implement the objective function for a slightly more challenging HPO problem, and to run more realistic experiments. We will use the two hidden layer MLP `DropoutMLP` implemented in @sec-dropout.
    ...
    3. Run random search on this example with `number_of_trials=20` and plot the results. Make sure to first evaluate the default configuration of @sec-dropout, which is `initial_config = {'num_hiddens_1': 256, 'num_hiddens_2': 256, 'dropout_1': 0.5, 'dropout_2': 0.5, 'lr': 0.1, 'batch_size': 256}`.
