...
hyperparameter optimization (see @distributed-scheduling). In the
...
@distributed-scheduling) before we can continue the optimization
process.
...
![Distributing the hyperparameter optimization process either synchronously or asynchronously. Compared to the sequential setting, we can reduce the overall wall-clock time while keep the total compute constant. Synchronous scheduling might lead to idling workers in the case of stragglers.](../img/distributed_scheduling.svg){#distributed-scheduling}
