...
# Asynchronous Random Search {#sec-rs-async}

As we have seen in the previous @sec-api-hpo, we might have to wait
...
hyperparameter optimization (see @fig-distributed-scheduling). In the
...
@fig-distributed-scheduling) before we can continue the optimization
process.
...
that make decision based on previous observations (see @sec-sh-async).
...
![Distributing the hyperparameter optimization process either synchronously or asynchronously. Compared to the sequential setting, we can reduce the overall wall-clock time while keep the total compute constant. Synchronous scheduling might lead to idling workers in the case of stragglers.](../img/distributed_scheduling.svg){#fig-distributed-scheduling}
...
[@salinas-automl22], which provides us with a simple interface for asynchronous
...
in behaviour to our `BasicScheduler` from @sec-api-hpo.
...
1. Consider the `DropoutMLP` model implemented in @sec-dropout, and used in Exercise 1 of @sec-api-hpo.
    ...
    2. Using the setup of Exercise 1 in @sec-api-hpo, compare random search to Bayesian optimization. If you use SageMaker, feel free to use Syne Tune's benchmarking facilities in order to run experiments in parallel. Hint: Bayesian optimization is provided as `syne_tune.optimizer.baselines.BayesianOptimization`.
...
2. *Advanced*. The goal of this exercise is to implement a new scheduler in Syne Tune.
    ...
    2. Implement the `LocalSearcher` from Exercise 2 in @sec-api-hpo as a new searcher in Syne Tune. Hint: Read [this tutorial](https://syne-tune.readthedocs.io/en/latest/tutorials/developer/README.html). Alternatively, you may follow this [example](https://syne-tune.readthedocs.io/en/latest/examples.html#launch-hpo-experiment-with-home-made-scheduler).
