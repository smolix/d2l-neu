...
# Asynchronous Successive Halving {#sec-sh-async}

As we have seen in @sec-rs-async, we can accelerate HPO by
...
Figure @fig-synchronous-sh shows the scheduling of synchronous SH with $\eta=2$
...
![Synchronous successive halving with two workers.](../img/sync_sh.svg){#fig-synchronous-sh}

Asynchronous successive halving (ASHA) [@li-arxiv18] adapts SH to the asynchronous
...
@fig-asha shows the scheduling of the same configurations for ASHA. Once Trial-1
...
![Asynchronous successive halving (ASHA) with two workers.](../img/asha.svg){#fig-asha}
...
@sec-mf-hpo-sh, where each training job is started with a fixed
...
asynchronous random search in @sec-rs-async. As we have seen for
successive halving in @sec-mf-hpo, most of the trials are stopped
