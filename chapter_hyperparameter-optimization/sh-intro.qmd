...
# Multi-Fidelity Hyperparameter Optimization {#sec-mf-hpo}

Training neural networks can be expensive even on moderate size datasets.
Depending on the configuration space (@sec-intro-config-spaces),
...
@sec-rs-async, we can significantly speed up the overall wall-clock
...
evaluation. @img-samples-lc depicts learning curves of a set of neural
...
![Learning curves of random hyperparameter configurations](../img/samples_lc.svg){#img-samples-lc}
...
More formally, we expand our definition in @sec-definition-hpo,
...
## Successive Halving {#sec-mf-hpo-sh}

One of the simplest ways to adapt random search to the multi-fidelity setting is
*successive halving* [@jamieson-aistats16; @karnin-icml13]. The basic
...
We subclass the `HPOScheduler` base class from @sec-api-hpo in order to
