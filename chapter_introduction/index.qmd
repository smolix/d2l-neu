...
as illustrated in @fig-wake-word.
...
As shown in @fig-ml-loop, the training process usually looks like the following:
...
The full process is drawn in @fig-supervised-learning.
...
as shown in @fig-death-cap.

![Death cap---do not eat!](../img/death-cap.jpg){#fig-death-cap width="200px"}
...
@fig-death-cap shows a death cap is 0.2.
...
![A donkey, a dog, a cat, and a rooster.](../img/stackedanimals.png){#fig-stackedanimals width="300px"}
...
@fig-deeplearning-amazon displays the deep learning books
...
is an audio recording of a speaker (@fig-speech),
...
![`-D-e-e-p- L-ea-r-ni-ng-` in an audio recording.](../img/speech.png){#fig-speech width="700px"}
...
came with the invention of *variational autoencoders* [@Kingma.Welling.2014; @rezende2014stochastic]
and continued with the development of *generative adversarial networks* [@Goodfellow.Pouget-Abadie.Mirza.ea.2014].
More recent advances include normalizing flows [@dinh2014nice; @dinh2017density] and
diffusion models [@sohl2015deep; @song2019generative; @ho2020denoising; @song2021score].
...
in big corpora without any labeling effort [@Devlin.Chang.Lee.ea.2018]!
For images, we may train models
to tell the relative position
between two cropped regions
of the same image [@Doersch.Gupta.Efros.2015],
...
depicted in @fig-data-collection.
...
at Atari games using only the visual input [@mnih2015human],
and the AlphaGo program, which dethroned the world champion
at the board game Go [@Silver.Huang.Maddison.ea.2016],
...
This process is illustrated in @fig-rl-environment.
...
to estimate the typical foot length in the population (@fig-koebel).

![Estimating the length of a foot.](../img/koebel.jpg){#fig-koebel width="500px"}
...
in his famous paper *Computing Machinery and Intelligence* [@Turing.1950].
...
In his groundbreaking book *The Organization of Behavior* [@Hebb.1949],
...
were within reach.
This is best illustrated in @tbl-intro-decade.
...
: Dataset vs. computer memory and computational power {#tbl-intro-decade}
...
[@McCulloch.Pitts.1943], convolutional neural networks
[@LeCun.Bottou.Bengio.ea.1998], long short-term memory
...
* Novel methods for capacity control, such as *dropout*
  ...
  Here, noise is injected [@Bishop.1995]
...
* *Attention mechanisms* solved a second problem
  ...
  a *learnable pointer structure* [@Bahdanau.Cho.Bengio.2014].
...
* Built solely on attention mechanisms,
  the *Transformer* architecture [@Vaswani.Shazeer.Parmar.ea.2017] has demonstrated superior *scaling* behavior: it performs better with an increase in dataset size, model size, and amount of training compute [@kaplan2020scaling]. This architecture has demonstrated compelling success in a wide range of areas,
  such as natural language processing [@Devlin.Chang.Lee.ea.2018; @brown2020language], computer vision [@Dosovitskiy.Beyer.Kolesnikov.ea.2021; @liu2021swin], speech recognition [@gulati2020conformer], reinforcement learning [@chen2021decision], and graph neural networks [@dwivedi2020generalization]. For example, a single Transformer pretrained on modalities
  as diverse as text, images, joint torques, and button presses
  can play Atari, caption images, chat,
  and control a robot [@reed2022generalist].
* Modeling probabilities of text sequences, *language models* can predict text given other text. Scaling up the data, model, and compute has unlocked a growing number of capabilities of language models to perform desired tasks via human-like text generation based on input text [@brown2020language; @rae2021scaling; @hoffmann2022training; @chowdhery2022palm; @openai2023gpt4; @anil2023palm; @touvron2023llama; @touvron2023llama2]. For instance, aligning language models with human intent [@ouyang2022training], OpenAI's [ChatGPT](https://chat.openai.com/) allows users to interact with it in a conversational way to solve problems, such as code debugging and creative writing.
* Multi-stage designs, e.g., via the memory networks
  [@Sukhbaatar.Weston.Fergus.ea.2015]
  and the neural programmer-interpreter [@Reed.De-Freitas.2015]
...
* A key development in *deep generative modeling* was the invention
  ...
  Examples of galloping zebras [@Zhu.Park.Isola.ea.2017]
  and of fake celebrity faces [@Karras.Aila.Laine.ea.2017]
  are each testimony to this progress.
  Even amateur doodlers can produce
  photorealistic images just based on sketches describing the layout of a scene [@Park.Liu.Wang.ea.2019]. 
* Furthermore, while the diffusion process gradually adds random noise to data samples, *diffusion models* [@sohl2015deep; @ho2020denoising] learn the denoising process to gradually construct data samples from random noise, reversing the diffusion process. They have started to replace generative adversarial networks in more recent deep generative models, such as in DALL-E 2 [@ramesh2022hierarchical] and Imagen [@saharia2022photorealistic] for creative art and image generation based on text descriptions.
* In many cases, a single GPU is insufficient for processing the large amounts of data available for training.
  ...
  and subsequently by @You.Gitman.Ginsburg.2017
  and @Jia.Song.He.ea.2018 pushed the size up to 64,000 observations,
...
* The ability to parallelize computation
  ...
  See, e.g., @Silver.Huang.Maddison.ea.2016 for a description
...
* A key ingredient in digital assistants
  ...
  for certain applications [@Xiong.Wu.Alleva.ea.2018].
* Object recognition has likewise come a long way.
  ...
  achieved a top-five error rate of 28% [@Lin.Lv.Zhu.ea.2010].
  By 2017, this error rate was reduced to 2.25% [@Hu.Shen.Sun.2018].
...
* Prowess in games used to provide
  ...
  through the game tree [@Campbell.Hoane-Jr.Hsu.2002].
  Go is more difficult still, due to its huge state space.
  AlphaGo reached human parity in 2015,
  using deep learning combined with Monte Carlo tree sampling [@Silver.Huang.Maddison.ea.2016].
  ...
  using efficiently structured strategies [@Brown.Sandholm.2017].
...
and Lowe's SIFT feature extractor [@Lowe.2004]
