# Environment and Distribution Shift {#sec-environment-and-distribution-shift}
...
Our training data might consist of images of the kind in @fig-cat-dog-train.

![Training data for distinguishing cats and dogs (illustrations: Lafeez Hossain / 500px / Getty Images; ilkermetinkursova / iStock / Getty Images Plus; GlobalP / iStock / Getty Images Plus; Musthafa Aboobakuru / 500px / Getty Images).](../img/cat-dog-train.png){#fig-cat-dog-train}


At test time we are asked to classify the images in @fig-cat-dog-test.

![Test data for distinguishing cats and dogs (illustrations: SIBAS_minich / iStock / Getty Images Plus; Ghrzuzudu / iStock / Getty Images Plus; id-work / DigitalVision Vectors / Getty Images; Yime / iStock / Getty Images Plus).](../img/cat-dog-test.png){#fig-cat-dog-test}
...
as shown in @fig-popvssoda.

![Concept shift for soft drink names in the United States (CC-BY: Alan McConchie, PopVsSoda.com).](../img/popvssoda.png){#fig-popvssoda width="400px"}
...
### Empirical Risk and  Risk {#sec-empirical-risk-and-risk}
...
$$\mathop{\mathrm{minimize}}_f \frac{1}{n} \sum_{i=1}^n l(f(\mathbf{x}_i), y_i),$$ {#eq-empirical-risk-min}

where $l$ is the loss function
measuring "how bad" the prediction $f(\mathbf{x}_i)$ is given the associated label $y_i$.
Statisticians call the term in @eq-empirical-risk-min *empirical risk*.
...
$$E_{p(\mathbf{x}, y)} [l(f(\mathbf{x}), y)] = \int\int l(f(\mathbf{x}), y) p(\mathbf{x}, y) \;d\mathbf{x}dy.$$ {#eq-true-risk}

However, in practice we typically cannot obtain the entire population of data.
Thus, *empirical risk minimization*,
which is minimizing the empirical risk in @eq-empirical-risk-min,
is a practical strategy for machine learning,
with the hope of approximately
minimizing the risk.


### Covariate Shift Correction {#sec-covariate-shift-correction}
...
$$\beta_i \stackrel{\textrm{def}}{=} \frac{p(\mathbf{x}_i)}{q(\mathbf{x}_i)}.$$ {#eq-environment-and-distribution-shift-auto-1}
...
$$\mathop{\mathrm{minimize}}_f \frac{1}{n} \sum_{i=1}^n \beta_i l(f(\mathbf{x}_i), y_i).$$ {#eq-weighted-empirical-risk-min}
...
which is a special case of softmax regression (see @sec-softmax)
...
in @eq-weighted-empirical-risk-min
...
1. Use weights $\beta_i$ for training on $\{(\mathbf{x}_1, y_1), \ldots, (\mathbf{x}_n, y_n)\}$ in @eq-weighted-empirical-risk-min.
...
Using the same notation in @sec-covariate-shift-correction,
...
$$\beta_i \stackrel{\textrm{def}}{=} \frac{p(y_i)}{q(y_i)}.$$ {#eq-environment-and-distribution-shift-auto-2}
...
$$\mathbf{C} p(\mathbf{y}) = \mu(\hat{\mathbf{y}}),$$ {#eq-environment-and-distribution-shift-auto-3}
...
$$\begin{aligned}&\textrm{model } f_t \longrightarrow \textrm{data }  \mathbf{x}_t \longrightarrow \textrm{estimate } f_t(\mathbf{x}_t) \longrightarrow\\ \textrm{obs}&\textrm{ervation } y_t \longrightarrow \textrm{loss } l(y_t, f_t(\mathbf{x}_t)) \longrightarrow \textrm{model } f_{t+1}\end{aligned}$$ {#eq-environment-and-distribution-shift-auto-4}
...
and improve the diversity of generated text and the reconstruction quality of generated images [@Shao.Yao.Sun.ea.2020].
