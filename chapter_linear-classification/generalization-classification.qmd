# Generalization in Classification {#sec-chap-classification-generalization}
...
Whereas @sec-generalization-basics introduced
...
When we get to @sec-chap-perceptrons,
...
$$\epsilon_{\mathcal{D}}(f) = \frac{1}{n}\sum_{i=1}^n \mathbf{1}(f(\mathbf{x}^{(i)}) \neq y^{(i)}).$$ {#eq-generalization-classification-auto-1}
...
\int\int \mathbf{1}(f(\mathbf{x}) \neq y) p(\mathbf{x}, y) \;d\mathbf{x} dy.$$ {#eq-generalization-classification-auto-2}
...
which you may recall from @sec-prob.
...
$$P(\epsilon_{\mathcal{D}}(f) - \epsilon(f) \geq t) < \exp\left( - 2n t^2 \right).$$ {#eq-generalization-classification-auto-3}
...
@sec-generalization-basics to heart
...
\ \textrm{ for }\ \alpha \geq c \sqrt{(\textrm{VC} - \log \delta)/n}.$$ {#eq-generalization-classification-auto-4}
...
See @boucheron2005theory for a detailed discussion
