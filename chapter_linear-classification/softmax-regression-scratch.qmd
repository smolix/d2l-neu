...
# Softmax Regression Implementation from Scratch {#sec-softmax-scratch}
...
as discussed in @sec-lin-alg-reduction
and @sec-lin-alg-non-reduction.
...
$$\mathrm{softmax}(\mathbf{X})_{ij} = \frac{\exp(\mathbf{X}_{ij})}{\sum_k \exp(\mathbf{X}_{ik})}.$$ {#eq-softmax-regression-scratch-auto-1}
...
(introduced in @sec-softmax-regression-loss-func).
...
We reuse the `fit` method defined in @sec-linear-scratch to train the model with 10 epochs.
...
on the *test* split.
As discussed in @sec-generalization-model-selection,
...
1. In this section, we directly implemented the softmax function based on the mathematical definition of the softmax operation. As discussed in @sec-softmax this can cause numerical instabilities.
