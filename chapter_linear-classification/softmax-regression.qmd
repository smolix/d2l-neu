...
In @sec-linear-regression, we introduced linear regression,
working through implementations from scratch in @sec-linear-scratch
and again using high-level APIs of a deep learning framework
in @sec-linear-concise to do the heavy lifting.
...
See @Tsoumakas.Katakis.2007 for an overview
and @Huang.Xu.Yu.2015
for an effective algorithm when tagging images.

## Classification {#sec-classification-problem}
...
See @Moon.Smola.Chang.ea.2010 for an overview
of different types of ranking loss functions
and @Beutel.Murray.Faloutsos.ea.2014 for a Bayesian approach
...
$$y \in \{(1, 0, 0), (0, 1, 0), (0, 0, 1)\}.$$ {#eq-softmax-regression-auto-1}
...
![Softmax regression is a single-layer neural network.](../img/softmaxreg.svg){#fig-softmaxreg}
...
### The Softmax {#sec-softmax-operation}
...
first introduced by @Fechner.1860 .
...
$$\hat{\mathbf{y}} = \mathrm{softmax}(\mathbf{o}) \quad \textrm{where}\quad \hat{y}_i = \frac{\exp(o_i)}{\sum_j \exp(o_j)}.$$ {#eq-softmax-y-and-o}
...
The idea of a softmax dates back to @Gibbs.1902,
...
Energy-based models [@Ranzato.Boureau.Chopra.ea.2007]
use this point of view when describing
problems in deep learning.

### Vectorization {#sec-softmax-vectorization}
...
$$ \begin{aligned} \mathbf{O} &= \mathbf{X} \mathbf{W} + \mathbf{b}, \\ \hat{\mathbf{Y}} & = \mathrm{softmax}(\mathbf{O}). \end{aligned} $$ {#eq-minibatch-softmax-reg}
...
## Loss Function {#sec-softmax-regression-loss-func}
...
@sec-normal-distribution-and-squared-loss.
...
$$ l(\mathbf{y}, \hat{\mathbf{y}}) = - \sum_{j=1}^q y_j \log \hat{y}_j. $$ {#eq-l-cross-entropy}

For reasons explained later on,
the loss function in @eq-l-cross-entropy
...
### Softmax and Cross-Entropy Loss {#sec-softmax-and-derivatives}
...
Plugging @eq-softmax-y-and-o into the definition of the loss
in @eq-l-cross-entropy
...
@Cover.Thomas.1999 or @mackay2003information.


## Information Theory Basics {#sec-info-theory-basics}
...
$$H[P] = \sum_j - P(j) \log P(j).$$ {#eq-softmax-reg-entropy}
...
The entropy defined in @eq-softmax-reg-entropy
...
For instance Deep Fried Convnets [@Yang.Moczulski.Denil.ea.2015]
...
structural matrix approximations [@sindhwani2015structured].
...
for computational and storage cost [@Zhang.Tay.Zhang.ea.2021]
...
the item with the largest score is the most likely one to be chosen [@Bradley.Terry.1952].
