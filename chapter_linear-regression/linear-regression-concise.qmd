...
# Concise Implementation of Linear Regression {#sec-linear-concise}
...
and Caffe [@Jia.Shelhamer.Donahue.ea.2014]
...
In @sec-linear-scratch, we relied only on
...
the linear regression model from @sec-linear-scratch
...
as described in @fig-single-neuron.
...
The `MSELoss` class computes the mean squared error (without the $1/2$ factor in @eq-mse).
By default, `MSELoss` returns the average loss over examples.
It is faster (and easier to use) than implementing our own.

## TensorFlow

The `MeanSquaredError` class computes the mean squared error (without the $1/2$ factor in @eq-mse).
...
while the `Trainer` class we created in @sec-oo-design
...
So we just call the `fit` method (introduced in @sec-oo-design-training),
...
such as MXNet [@Chen.Li.Li.ea.2015], 
JAX [@Frostig.Johnson.Leary.2018], 
PyTorch [@Paszke.Gross.Massa.ea.2019], 
and Tensorflow [@Abadi.Barham.Chen.ea.2016].
...
1. Review the framework documentation to see which loss functions are provided. In particular,
   ...
   $$l(y,y') = \begin{cases}|y-y'| -\frac{\sigma}{2} & \textrm{ if } |y-y'| > \sigma \\ \frac{1}{2 \sigma} (y-y')^2 & \textrm{ otherwise}\end{cases}$$ {#eq-linear-regression-concise-auto-1}
