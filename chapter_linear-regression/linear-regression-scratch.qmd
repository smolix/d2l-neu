...
# Linear Regression Implementation from Scratch {#sec-linear-scratch}
...
from @sec-synthetic-regression-data
and apply our model
...
we add the code to the `__init__` method of a subclass of `d2l.Module` (introduced in @sec-oo-design-models).
...
Using the same notation as @eq-linreg-y-vec
...
via `add_to_class` (introduced in @sec-oo-design-utilities).
...
As discussed in @sec-linear-regression,
...
a subclass of `d2l.HyperParameters` (introduced in @sec-oo-design-utilities),
...
that we generated in @sec-synthetic-regression-data 
...
(introduced in @sec-oo-design-training).
