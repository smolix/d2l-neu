...
from @sec-linear-regression for linear regression.
There, our loss was given by

$$L(\mathbf{w}, b) = \frac{1}{n}\sum_{i=1}^n \frac{1}{2}\left(\mathbf{w}^\top \mathbf{x}^{(i)} + b - y^{(i)}\right)^2.$$ {#eq-weight-decay-auto-1}
...
$$L(\mathbf{w}, b) + \frac{\lambda}{2} \|\mathbf{w}\|^2.$$ {#eq-weight-decay-auto-2}
...
\epsilon \sim \mathcal{N}(0, 0.01^2).$$ {#eq-weight-decay-auto-4}
...
In the final model,
the linear regression and the squared loss have not changed since @sec-linear-scratch,
...
1. We know that $\|\mathbf{w}\|^2 = \mathbf{w}^\top \mathbf{w}$. Can you find a similar equation for matrices (see the Frobenius norm in @sec-lin-algebra-norms)?
