...
**Diagrams:** `fig_mdl-convex-vs-nonconvex-set` — a convex blob with an interior chord vs. a non-convex (crescent/star) set whose chord exits, plus the simplex triangle and a half-space.
...
**Diagrams:** `fig_mdl-chord-above-graph` — a convex curve with a chord lying above it and a tangent lying below it, annotating both the chord and first-order inequalities on one picture.
...
**Diagrams:** reuse `fig_mdl-chord-above-graph`; annotate $\mathbb{E}[f(X)]$ above $f(\mathbb{E}[X])$ for a two-mass distribution.
...
**Diagrams:** `fig_mdl-local-equals-global` — a convex bowl (single global min) beside a non-convex landscape with multiple local minima and a saddle, arrows showing GD trapped on the right but not the left.
...
**Diagrams:** none new; reference `fig_mdl-chord-above-graph` for the pointwise-max picture (upper envelope of lines is convex).
...
**Diagrams:** reuse `fig_mdl-local-equals-global` (the non-convex panel) with a PL annotation; no new SVG.
