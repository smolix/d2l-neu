# Forward Propagation, Backward Propagation, and Computational Graphs {#sec-backprop}
...
$$\mathbf{z}= \mathbf{W}^{(1)} \mathbf{x},$$ {#eq-backprop-auto-1}
...
$$\mathbf{h}= \phi (\mathbf{z}).$$ {#eq-backprop-auto-2}
...
$$\mathbf{o}= \mathbf{W}^{(2)} \mathbf{h}.$$ {#eq-backprop-auto-3}
...
$$L = l(\mathbf{o}, y).$$ {#eq-backprop-auto-4}
...
$$s = \frac{\lambda}{2} \left(\|\mathbf{W}^{(1)}\|_\textrm{F}^2 + \|\mathbf{W}^{(2)}\|_\textrm{F}^2\right),$$ {#eq-forward-s}
...
@fig-forward contains the graph associated
...
![Computational graph of forward propagation.](../img/forward.svg){#fig-forward}
...
$$\frac{\partial \mathsf{Z}}{\partial \mathsf{X}} = \textrm{prod}\left(\frac{\partial \mathsf{Z}}{\partial \mathsf{Y}}, \frac{\partial \mathsf{Y}}{\partial \mathsf{X}}\right).$$ {#eq-backprop-auto-5}
...
whose computational graph is in @fig-forward,
...
$$\frac{\partial J}{\partial L} = 1 \; \textrm{and} \; \frac{\partial J}{\partial s} = 1.$$ {#eq-backprop-auto-6}
...
\; \textrm{and} \;
\frac{\partial s}{\partial \mathbf{W}^{(2)}} = \lambda \mathbf{W}^{(2)}.$$ {#eq-backprop-auto-7}
...
$$\frac{\partial J}{\partial \mathbf{W}^{(2)}}= \textrm{prod}\left(\frac{\partial J}{\partial \mathbf{o}}, \frac{\partial \mathbf{o}}{\partial \mathbf{W}^{(2)}}\right) + \textrm{prod}\left(\frac{\partial J}{\partial s}, \frac{\partial s}{\partial \mathbf{W}^{(2)}}\right)= \frac{\partial J}{\partial \mathbf{o}} \mathbf{h}^\top + \lambda \mathbf{W}^{(2)}.$$ {#eq-backprop-J-h}
...
On the one hand,
computing the regularization term @eq-forward-s
...
@eq-backprop-J-h during backpropagation
