...
($\ell_2$ regularization) in @sec-weight-decay,
...
In @fig-dropout2, $h_2$ and $h_5$ are removed.
...
![MLP before and after dropout.](../img/dropout2.svg){#fig-dropout2}
...
@sec-oo-design-training) as an attribute and with every epoch
it is replaced with a new `dropout_rng`. We already handled this with the
`fit_epoch` method defined in @sec-linear-scratch.
