...
In @sec-chap-regression and @sec-chap-classification,
...
According to the "no free lunch" theorem of @wolpert1995no,
...
(@fig-capacity-vs-error)
from @sec-generalization-basics.
...
is the $k$-nearest neighbor algorithm (we will cover more nonparametric models later, for example in @sec-attention-pooling).
...
In particular, @Jacot.Grabriel.Hongler.2018
...
A new line of work [@Rolnick.Veit.Belongie.Shavit.2017]
...
it has in fact generalized [@Garg.Balakrishnan.Kolter.Lipton.2021].
...
from interpolating the data [@zhang2021understanding] and thus the benefits if interpreted
...
(invented by @Srivastava.Hinton.Krizhevsky.ea.2014),
