...
In @sec-softmax, we introduced
softmax regression,
implementing the algorithm from scratch
(@sec-softmax-scratch) and using high-level APIs
(@sec-softmax-concise). This allowed us to
...
[@Aronszajn.1950]. This has found its way into
nonparametric spline models [@Wahba.1990] and kernel methods
[@Scholkopf.Smola.2002]. It is also something that the brain solves
...
often abbreviated as *MLP* (@fig-mlp).

![An MLP with a hidden layer of five hidden units.](../img/mlp.svg){#fig-mlp}
...
when we denoted a rowwise operation in @sec-softmax-vectorization.
...
has been answered multiple times, e.g., in @Cybenko.1989 in the context
of MLPs, and in @micchelli1984interpolation in the context of reproducing kernel
...
*exactly* even in infinite dimensional spaces [@Kimeldorf.Wahba.1971; @Scholkopf.Herbrich.Smola.2001].
In fact, we can approximate many functions
much more compactly by using deeper (rather than wider) networks [@Simonyan.Zisserman.2014].
...
$$\operatorname{ReLU}(x) = \max(x, 0).$$ {#eq-mlp-auto-1}
...
we are not performing constrained optimization [@Mangasarian.1965; @Rockafellar.1970].
...
including the *parametrized ReLU* (*pReLU*) function [@He.Zhang.Ren.ea.2015].
This variation adds a linear term to ReLU,
so some information still gets through,
even when the argument is negative:

$$\operatorname{pReLU}(x) = \max(0, x) + \alpha \min(0, x).$$ {#eq-mlp-auto-2}
...
$$\operatorname{sigmoid}(x) = \frac{1}{1 + \exp(-x)}.$$ {#eq-mlp-auto-3}
...
Nonetheless sigmoids are important. In later chapters (e.g., @sec-lstm) on recurrent neural networks,
...
$$\frac{d}{dx} \operatorname{sigmoid}(x) = \frac{\exp(-x)}{(1 + \exp(-x))^2} = \operatorname{sigmoid}(x)\left(1-\operatorname{sigmoid}(x)\right).$$ {#eq-mlp-auto-4}
...
$$\operatorname{tanh}(x) = \frac{1 - \exp(-2x)}{1 + \exp(-2x)}.$$ {#eq-mlp-auto-5}
...
$$\frac{d}{dx} \operatorname{tanh}(x) = 1 - \operatorname{tanh}^2(x).$$ {#eq-mlp-auto-6}
...
activation function $x \Phi(x)$ by @Hendrycks.Gimpel.2016 ($\Phi(x)$
is the standard Gaussian cumulative distribution function) 
and
the Swish activation
function $\sigma(x) = x \operatorname{sigmoid}(\beta x)$ as proposed in @Ramachandran.Zoph.Le.2017 can yield better accuracy
in many cases.
...
1. Assume that we have a nonlinearity that applies to one minibatch at a time, such as the batch normalization [@Ioffe.Szegedy.2015]. What kinds of problems do you expect this to cause?
