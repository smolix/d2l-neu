...
# Numerical Stability and Initialization {#sec-numerical-stability}
...
$$\mathbf{h}^{(l)} = f_l (\mathbf{h}^{(l-1)}) \textrm{ and thus } \mathbf{o} = f_L \circ \cdots \circ f_1(\mathbf{x}).$$ {#eq-numerical-stability-and-init-auto-1}
...
$$\partial_{\mathbf{W}^{(l)}} \mathbf{o} = \underbrace{\partial_{\mathbf{h}^{(L-1)}} \mathbf{h}^{(L)}}_{ \mathbf{M}^{(L)} \stackrel{\textrm{def}}{=}} \cdots \underbrace{\partial_{\mathbf{h}^{(l)}} \mathbf{h}^{(l+1)}}_{ \mathbf{M}^{(l+1)} \stackrel{\textrm{def}}{=}} \underbrace{\partial_{\mathbf{W}^{(l)}} \mathbf{h}^{(l)}}_{ \mathbf{v}^{(l)} \stackrel{\textrm{def}}{=}}.$$ {#eq-numerical-stability-and-init-auto-2}
...
$1/(1 + \exp(-x))$ (introduced in @sec-mlp)
...
In the previous sections, e.g., in @sec-linear-concise,
...
### Xavier Initialization {#sec-xavier}
...
$$o_{i} = \sum_{j=1}^{n_\textrm{in}} w_{ij} x_j.$$ {#eq-numerical-stability-and-init-auto-3}
...
$$U\left(-\sqrt{\frac{6}{n_\textrm{in} + n_\textrm{out}}}, \sqrt{\frac{6}{n_\textrm{in} + n_\textrm{out}}}\right).$$ {#eq-numerical-stability-and-init-auto-4}
