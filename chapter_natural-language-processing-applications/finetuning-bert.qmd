# Fine-Tuning BERT for Sequence-Level and Token-Level Applications {#sec-finetuning-bert}
...
On the other hand,
as noted in @sec-bert-pretraining,
...
judging whether a given sentence is grammatically acceptable or not [@Warstadt.Singh.Bowman.2019].
For instance, "I should study." is acceptable but "I should studying." is not.

![Fine-tuning BERT for single text classification applications, such as sentiment analysis and testing linguistic acceptability. Suppose that the input single text has six tokens.](../img/bert-one-seq.svg){#fig-bert-one-seq}

@sec-bert describes the input representation of BERT.
...
is an ordinal scale ranging from 0 (no meaning overlap) to 5 (meaning equivalence) [@Cer.Diab.Agirre.ea.2017].
...
![Fine-tuning BERT for text pair classification or regression applications, such as natural language inference and semantic textual similarity. Suppose that the input text pair has two and three tokens.](../img/bert-two-seqs.svg){#fig-bert-two-seqs}

Comparing with single text classification in @fig-bert-one-seq,
fine-tuning BERT for text pair classification in @fig-bert-two-seqs 
...
![Fine-tuning BERT for text tagging applications, such as part-of-speech tagging. Suppose that the input single text has six tokens.](../img/bert-tagging.svg){#fig-bert-tagging}

Fine-tuning BERT for text tagging applications
is illustrated in @fig-bert-tagging.
Comparing with @fig-bert-one-seq,
...
is just a segment of text (text span) from the passage that the question is about [@Rajpurkar.Zhang.Lopyrev.ea.2016].
To explain,
consider a passage
...
![Fine-tuning BERT for question answering. Suppose that the input text pair has two and three tokens.](../img/bert-qa.svg){#fig-bert-qa}
