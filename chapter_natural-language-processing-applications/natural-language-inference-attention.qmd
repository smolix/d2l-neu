# Natural Language Inference: Using Attention {#sec-natural-language-inference-attention}

We introduced the natural language inference task and the SNLI dataset in @sec-natural-language-inference-and-dataset. In view of many models that are based on complex and deep architectures, @Parikh.Tackstrom.Das.ea.2016 proposed to address natural language inference with attention mechanisms and called it a "decomposable attention model".
...
In this section, we will describe and implement this attention-based method (with MLPs) for natural language inference, as depicted in @fig-nlp-map-nli-attention.

![This section feeds pretrained GloVe to an architecture based on attention and MLPs for natural language inference.](../img/nlp-map-nli-attention.svg){#fig-nlp-map-nli-attention}
...
![Natural language inference using attention mechanisms.](../img/nli-attention.svg){#fig-nli-attention}

@fig-nli-attention depicts the natural language inference method using attention mechanisms.
...
For ease of demonstration, @fig-nli-attention shows such alignment in a *hard* way.
...
$$e_{ij} = f(\mathbf{a}_i)^\top f(\mathbf{b}_j),$$ {#eq-nli-e}
...
It should be highlighted that, in @eq-nli-e
...
Normalizing the attention weights in @eq-nli-e,
...
For easy of demonstration, @fig-nli-attention pairs tokens with aligned tokens in a *hard* way.
...
$$\mathbf{v}_{A,i} = g([\mathbf{a}_i, \boldsymbol{\beta}_i]), i = 1, \ldots, m\\ \mathbf{v}_{B,j} = g([\mathbf{b}_j, \boldsymbol{\alpha}_j]), j = 1, \ldots, n.$$ {#eq-nli-v-ab}


In @eq-nli-v-ab, $\mathbf{v}_{A,i}$ is the comparison between token $i$ in the premise and all the hypothesis tokens that are softly aligned with token $i$;
...
We download and read the SNLI dataset using the function defined in @sec-natural-language-inference-and-dataset. The batch size and sequence length are set to $256$ and $50$, respectively.
...
Thus, we predefine the dimension of vectors $\mathbf{a}_i$ and $\mathbf{b}_j$ in @eq-nli-e as 100.
The output dimension of functions $f$ in @eq-nli-e and $g$ in @eq-nli-v-ab is set to 200.
...
In contrast to the `split_batch` function in @sec-multi-gpu that takes single inputs such as text sequences (or images),
