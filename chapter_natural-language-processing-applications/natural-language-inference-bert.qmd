# Natural Language Inference: Fine-Tuning BERT {#sec-natural-language-inference-bert}

In earlier sections of this chapter,
we have designed an attention-based architecture
(in @sec-natural-language-inference-attention)
for the natural language inference task
on the SNLI dataset (as described in @sec-natural-language-inference-and-dataset).
Now we revisit this task by fine-tuning BERT.
As discussed in @sec-finetuning-bert,
...
as illustrated in @fig-nlp-map-nli-bert.

![This section feeds pretrained BERT to an MLP-based architecture for natural language inference.](../img/nlp-map-nli-bert.svg){#fig-nlp-map-nli-bert}
...
@sec-bert-dataset and @sec-bert-pretraining
(note that the original BERT model is pretrained on much bigger corpora).
As discussed in @sec-bert-pretraining,
...
and is packed into one BERT input sequence as depicted in @fig-bert-two-seqs.
Recall @sec-bert-input-rep that segment IDs
...
As @fig-bert-two-seqs indicates,
