# The Dataset for Pretraining BERT {#sec-bert-dataset}

To pretrain the BERT model as implemented in @sec-bert,
...
two huge corpora BookCorpus and English Wikipedia (see @sec-bert-pretraining-tasks),
...
we use a smaller corpus WikiText-2 [@Merity.Xiong.Bradbury.ea.2016].

Comparing with the PTB dataset used for pretraining word2vec in @sec-word2vec-data,
...
According to descriptions of @sec-nsp,
...
### Generating the Masked Language Modeling Task {#sec-prepare-mlm-data}
...
Following the definition of the masked language modeling task in @sec-mlm,
...
(after possible token replacement as described in @sec-mlm),
...
The original BERT model uses WordPiece embeddings whose vocabulary size is 30000 [@Wu.Schuster.Chen.ea.2016].
The tokenization method of WordPiece is a slight modification of
the original byte pair encoding algorithm in @sec-Byte-Pair-Encoding.
