# Bidirectional Encoder Representations from Transformers (BERT) {#sec-bert}
...
Recall the experiments in @sec-word2vec-pretraining and @sec-synonyms.
...
include TagLM (language-model-augmented sequence tagger) [@Peters.Ammar.Bhagavatula.ea.2017],
CoVe (Context Vectors) [@McCann.Bradbury.Xiong.ea.2017],
and ELMo (Embeddings from Language Models) [@Peters.Neumann.Iyyer.ea.2018].
...
a general *task-agnostic* model for context-sensitive representations [@Radford.Narasimhan.Salimans.ea.2018].
...
for a wide range of natural language processing tasks [@Devlin.Chang.Lee.ea.2018].
...
@fig-elmo-gpt-bert depicts the differences among ELMo, GPT, and BERT.

![A comparison of ELMo, GPT, and BERT.](../img/elmo-gpt-bert.svg){#fig-elmo-gpt-bert}
...
When natural language processing applications are explained in @sec-chap-nlp-app,
...
## Input Representation {#sec-bert-input-rep}
...
To sum up, @fig-bert-input shows that
...
![The embeddings of the BERT input sequence are the sum
of the token embeddings, segment embeddings, and positional embeddings.](../img/bert-input.svg){#fig-bert-input}

The following `BERTEncoder` class is similar to the `TransformerEncoder` class
as implemented in @sec-transformer.
...
## Pretraining Tasks {#sec-bert-pretraining-tasks}
...
### Masked Language Modeling {#sec-mlm}

As illustrated in @sec-language-model,
...
### Next Sentence Prediction {#sec-nsp}
...
The original BERT has been pretrained on the concatenation of BookCorpus [@Zhu.Kiros.Zemel.ea.2015]
...
1. In the original implementation of BERT, the positionwise feed-forward network in `BERTEncoder` (via `d2l.TransformerEncoderBlock`) and the fully connected layer in `MaskLM` both use the Gaussian error linear unit (GELU) [@Hendrycks.Gimpel.2016] as the activation function. Research into the difference between GELU and ReLU.
