# Word Embedding with Global Vectors (GloVe) {#sec-glove}
...
for word embedding,
let's first revisit
the skip-gram model in @sec-skip-gram,
but interpreting it
using global corpus statistics
such as co-occurrence counts.

## Skip-Gram with Global Corpus Statistics {#sec-skipgram-global}
...
$$q_{ij}=\frac{\exp(\mathbf{u}_j^\top \mathbf{v}_i)}{ \sum_{k \in \mathcal{V}} \exp(\mathbf{u}_k^\top \mathbf{v}_i)},$$ {#eq-glove-auto-1}
...
$$-\sum_{i\in\mathcal{V}}\sum_{j\in\mathcal{V}} x_{ij} \log\,q_{ij}.$$ {#eq-skipgram-x-ij}
...
$$-\sum_{i\in\mathcal{V}} x_i \sum_{j\in\mathcal{V}} p_{ij} \log\,q_{ij}.$$ {#eq-skipgram-p-ij}

In @eq-skipgram-p-ij, $-\sum_{j\in\mathcal{V}} p_{ij} \log\,q_{ij}$ calculates
the cross-entropy
of
...
On the one hand, as we mentioned in @sec-approx-train,
...
to the skip-gram model based on squared loss [@Pennington.Socher.Manning.2014]:
...
$$\sum_{i\in\mathcal{V}} \sum_{j\in\mathcal{V}} h(x_{ij}) \left(\mathbf{u}_j^\top \mathbf{v}_i + b_i + c_j - \log\,x_{ij}\right)^2.$$ {#eq-glove-loss}
...
: Word-word co-occurrence probabilities and their ratios from a large corpus (adapted from Table 1 in @Pennington.Socher.Manning.2014) {#tbl-glove}


We can observe the following from @tbl-glove:
...
$$f(\mathbf{u}_j, \mathbf{u}_k, {\mathbf{v}}_i) \approx \frac{p_{ij}}{p_{ik}}.$$ {#eq-glove-f}
...
$$\mathbf{u}_j^\top \mathbf{v}_i + b_i + c_j \approx \log\, x_{ij}.$$ {#eq-glove-square}

Measuring the squared error of
@eq-glove-square with weights,
the GloVe loss function in
@eq-glove-loss is obtained.
...
1. If words $w_i$ and $w_j$ co-occur in the same context window, how can we use their   distance in the text sequence to redesign the method for  calculating the conditional probability $p_{ij}$? Hint: see Section 4.2 of the GloVe paper [@Pennington.Socher.Manning.2014].
