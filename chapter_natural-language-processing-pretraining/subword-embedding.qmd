# Subword Embedding {#sec-fasttext}
...
where a subword is a character $n$-gram [@Bojanowski.Grave.Joulin.ea.2017].
...
$$\mathbf{v}_w = \sum_{g\in\mathcal{G}_w} \mathbf{z}_g.$$ {#eq-subword-embedding-auto-1}
...
called *byte pair encoding* (BPE) to extract subwords [@Sennrich.Haddow.Birch.2015].
...
Byte pair encoding and its variants has been used for input representations in popular natural language processing pretraining models such as GPT-2 [@Radford.Wu.Child.ea.2019] and RoBERTa [@Liu.Ott.Goyal.ea.2019].
...
1. As an example, there are about $3\times 10^8$ possible  $6$-grams in English. What is the issue when there are too many subwords? How to address the issue? Hint: refer to the end of Section 3.2 of the fastText paper [@Bojanowski.Grave.Joulin.ea.2017].
