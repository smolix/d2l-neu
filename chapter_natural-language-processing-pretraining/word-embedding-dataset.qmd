# The Dataset for Pretraining Word Embeddings {#sec-word2vec-data}
...
Specifically,
we will take the skip-gram model in @sec-word2vec
and negative sampling in @sec-approx-train
as an example.
In this section,
...
high-frequency words can be *subsampled* [@Mikolov.Sutskever.Chen.ea.2013].
Specifically, 
each indexed word $w_i$ 
in the dataset will be discarded with probability


$$ P(w_i) = \max\left(1 - \sqrt{\frac{t}{f(w_i)}}, 0\right),$$ {#eq-word-embedding-dataset-auto-1}
...
in the dictionary
raised to 
the power of 0.75 [@Mikolov.Sutskever.Chen.ea.2013].
...
## Loading Training Examples in Minibatches {#sec-word2vec-minibatch-loading}
