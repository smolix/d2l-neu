...
Furthermore, even though the optimization problems in deep learning are generally nonconvex, they often exhibit some properties of convex ones near local minima. This can lead to exciting new optimization variants such as [@Izmailov.Podoprikhin.Garipov.ea.2018].
...
$$\lambda  a + (1-\lambda)  b \in \mathcal{X} \textrm{ whenever } a, b \in \mathcal{X}.$$ {#eq-convexity-auto-1}

This sounds a bit abstract. Consider @fig-pacman. The first set is not convex since there exist line segments that are not contained in it.
The other two sets suffer no such problem.

![The first set is nonconvex and the other two are convex.](../img/pacman.svg){#fig-pacman}

Definitions on their own are not particularly useful unless you can do something with them.
In this case we can look at intersections as shown in @fig-convex-intersect.
...
![The intersection between two convex sets is convex.](../img/convex-intersect.svg){#fig-convex-intersect}
...
$$\lambda f(x) + (1-\lambda) f(x') \geq f(\lambda x + (1-\lambda) x').$$ {#eq-convexity-auto-2}
...
$$\sum_i \alpha_i f(x_i)  \geq f\left(\sum_i \alpha_i x_i\right)    \textrm{ and }    E_X[f(X)]  \geq f\left(E_X[X]\right),$$ {#eq-jensens-inequality}
...
$$\begin{aligned}
    ...
    &= f(x^{\ast}),
\end{aligned}$$ {#eq-convexity-auto-3}
...
$$\mathcal{S}_b \stackrel{\textrm{def}}{=} \{x | x \in \mathcal{X} \textrm{ and } f(x) \leq b\}$$ {#eq-convexity-auto-4}
...
$$f(\lambda x + (1-\lambda) x') \leq \lambda f(x) + (1-\lambda) f(x') \leq b.$$ {#eq-convexity-auto-5}
...
$$\frac{1}{2} f(x + \epsilon) + \frac{1}{2} f(x - \epsilon) \geq f\left(\frac{x + \epsilon}{2} + \frac{x - \epsilon}{2}\right) = f(x).$$ {#eq-convexity-auto-6}

Since the second derivative is given by the limit over finite differences it follows that

$$f''(x) = \lim_{\epsilon \to 0} \frac{f(x+\epsilon) + f(x - \epsilon) - 2f(x)}{\epsilon^2} \geq 0.$$ {#eq-convexity-auto-7}
...
$$f'(\alpha) = \frac{f(x) - f(a)}{x-a} \textrm{ and } f'(\beta) = \frac{f(b) - f(x)}{b-x}.$$ {#eq-convexity-auto-8}


By monotonicity $f'(\beta) \geq f'(\alpha)$, hence

$$\frac{x-a}{b-a}f(b) + \frac{b-x}{b-a}f(a) \geq f(x).$$ {#eq-convexity-auto-9}

Since $x = (1-\lambda)a + \lambda b$,
we have

$$\lambda f(b) + (1-\lambda)f(a) \geq f((1-\lambda)a + \lambda b),$$ {#eq-convexity-auto-10}
...
$$g(z) \stackrel{\textrm{def}}{=} f(z \mathbf{x} + (1-z)  \mathbf{y}) \textrm{ where } z \in [0,1]$$  {#eq-convexity-auto-11}
...
\end{aligned}$$ {#eq-convexity-auto-12}
...
\end{aligned}$$ {#eq-convexity-auto-13}
...
\end{aligned}$$ {#eq-convexity-auto-14}
...
$$L(\mathbf{x}, \alpha_1, \ldots, \alpha_n) = f(\mathbf{x}) + \sum_{i=1}^n \alpha_i c_i(\mathbf{x}) \textrm{ where } \alpha_i \geq 0.$$ {#eq-convexity-auto-15}
...
An alternative strategy for satisfying constraints is projections. Again, we encountered them before, e.g., when dealing with gradient clipping in @sec-rnn-scratch. There we ensured that a gradient has length bounded by $\theta$ via

$$\mathbf{g} \leftarrow \mathbf{g} \cdot \mathrm{min}(1, \theta/\|\mathbf{g}\|).$$ {#eq-convexity-auto-16}
...
$$\textrm{Proj}_{\mathcal{X}}(\mathbf{x}) = \mathop{\mathrm{argmin}}_{\mathbf{x}' \in \mathcal{X}} \|\mathbf{x} - \mathbf{x}'\|,$$ {#eq-convexity-auto-17}

which is the closest point in $\mathcal{X}$ to $\mathbf{x}$. 

![Convex Projections.](../img/projections.svg){#fig-projections}

The mathematical definition of projections may sound a bit abstract. @fig-projections explains it somewhat more clearly. In it we have two convex sets, a circle and a diamond. 
...
which is a generalized version of the diamond case in @fig-projections.
