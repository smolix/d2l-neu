# Minibatch Stochastic Gradient Descent {#sec-minibatch-sgd}

So far we encountered two extremes in the approach to gradient-based learning: @sec-gd uses the full dataset to compute gradients and to update parameters, one pass at a time. Conversely @sec-sgd processes one training example at a time to make progress.
...
$$\mathbf{g}_t = \partial_{\mathbf{w}} f(\mathbf{x}_{t}, \mathbf{w})$$ {#eq-minibatch-sgd-auto-1}
...
$$\mathbf{g}_t = \partial_{\mathbf{w}} \frac{1}{|\mathcal{B}_t|} \sum_{i \in \mathcal{B}_t} f(\mathbf{x}_{i}, \mathbf{w})$$ {#eq-minibatch-sgd-auto-2}
