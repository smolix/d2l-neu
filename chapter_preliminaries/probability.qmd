...
$$P(A=a, B=b) \leq P(A=a) \textrm{ and } P(A=a, B=b) \leq P(B = b),$$ {#eq-probability-auto-1}
...
$$P(B=b \mid A=a) = P(A=a,B=b)/P(A=a).$$ {#eq-probability-auto-2}
...
$$P(A \mid B) = \frac{P(B\mid A) P(A)}{P(B)}.$$ {#eq-probability-auto-3}
...
$$P(A \mid B) \propto P(B \mid A) P(A).$$ {#eq-probability-auto-4}

Since we know that $P(A \mid B)$ must be normalized to $1$, i.e., $\sum_a P(A=a \mid B) = 1$,
we can use it to compute

$$P(A \mid B) = \frac{P(B \mid A) P(A)}{\sum_a P(B \mid A=a) P(A = a)}.$$ {#eq-probability-auto-5}
...
$$\sum_a P(B \mid A=a) P(A=a) = \sum_a P(B, A=a) = P(B).$$ {#eq-probability-auto-6}
...
## An Example {#sec-probability-hiv-app}
...
=& 0.011485.
\end{aligned}
$$ {#eq-probability-auto-7}

This leads us to

$$P(H = 1 \mid D_1 = 1) = \frac{P(D_1=1 \mid H=1) P(H=1)}{P(D_1=1)} = 0.1306.$$ {#eq-probability-auto-8}
...
$$E[X] = E_{x \sim P}[x] = \sum_{x} x P(X = x).$$ {#eq-probability-auto-12}
...
$$E_{x \sim P}[f(x)] = \sum_x f(x) P(x) \textrm{ and } E_{x \sim P}[f(x)] = \int f(x) p(x) \;dx$$ {#eq-probability-auto-13}
...
$$\textrm{Var}[X] = E\left[(X - E[X])^2\right] = E[X^2] - E[X]^2.$$ {#eq-probability-auto-14}
...
$$\textrm{Var}_{x \sim P}[f(x)] = E_{x \sim P}[f^2(x)] - E_{x \sim P}[f(x)]^2.$$ {#eq-probability-auto-15}
...
$$\boldsymbol{\Sigma} \stackrel{\textrm{def}}{=} \textrm{Cov}_{\mathbf{x} \sim P}[\mathbf{x}] = E_{\mathbf{x} \sim P}\left[(\mathbf{x} - \boldsymbol{\mu}) (\mathbf{x} - \boldsymbol{\mu})^\top\right].$$ {#eq-probability-auto-16}
...
$$\mathbf{v}^\top \boldsymbol{\Sigma} \mathbf{v} = E_{\mathbf{x} \sim P}\left[\mathbf{v}^\top(\mathbf{x} - \boldsymbol{\mu}) (\mathbf{x} - \boldsymbol{\mu})^\top \mathbf{v}\right] = \textrm{Var}_{x \sim P}[\mathbf{v}^\top \mathbf{x}].$$ {#eq-probability-auto-17}
...
(see e.g., @Der-Kiureghian.Ditlevsen.2009 for a review on this aspect of [uncertainty quantification](https://en.wikipedia.org/wiki/Uncertainty_quantification)).
...
For an empirical review of this fact for large scale language models see @Revels.Lubin.Papamarkou.2016 .
...
1. In @sec-probability-hiv-app, assume that the outcomes of the two tests are not independent. In particular assume that either test on its own has a false positive rate of 10% and a false negative rate of 1%. That is, assume that $P(D =1 \mid H=0) = 0.1$ and that $P(D = 0 \mid H=1) = 0.01$. Moreover, assume that for $H = 1$ (infected) the test outcomes are conditionally independent, i.e., that $P(D_1, D_2 \mid H=1) = P(D_1 \mid H=1) P(D_2 \mid H=1)$ but that for healthy patients the outcomes are coupled via $P(D_1 = D_2 = 1 \mid H=0) = 0.02$.
