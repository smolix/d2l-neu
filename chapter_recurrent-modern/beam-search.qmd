...
borrowing conventions from @sec-seq2seq.
...
Consider the simple *greedy search* strategy from @sec-seq2seq.
Here, at any time step $t'$, 
we simply select the token 
with the highest conditional probability
from $\mathcal{Y}$, i.e., 

$$y_{t'} = \operatorname*{argmax}_{y \in \mathcal{Y}} P(y \mid y_1, \ldots, y_{t'-1}, \mathbf{c}).$$ {#eq-beam-search-auto-1}
...
![At each time step, greedy search selects the token with the highest conditional probability.](../img/s2s-prob1.svg){#fig-s2s-prob1}
...
will be predicted (@fig-s2s-prob1). 
The conditional probability of this output sequence
is $0.5\times0.4\times0.4\times0.6 = 0.048$.


Next, let's look at another example in @fig-s2s-prob2. 
Unlike in @fig-s2s-prob1, 
...
is selected.](../img/s2s-prob2.svg){#fig-s2s-prob2}

Since the output subsequences at time steps 1 and 2, 
on which time step 3 is based, 
have changed from "A" and "B" in @fig-s2s-prob1 
to "A" and "C" in @fig-s2s-prob2, 
the conditional probability of each token 
at time step 3 has also changed in @fig-s2s-prob2. 
...
"A", "C", and "B", 
which has changed from "A", "B", and "C" in @fig-s2s-prob1. 
Therefore, the conditional probability of generating 
each token at time step 4 in @fig-s2s-prob2 
is also different from that in @fig-s2s-prob1. 
As a result, the conditional probability of the output sequence 
"A", "C", "B", and "&lt;eos&gt;" in @fig-s2s-prob2
is $0.5\times0.3 \times0.6\times0.6=0.054$, 
which is greater than that of greedy search in @fig-s2s-prob1. 
...
![The process of beam search (beam size $=2$; maximum length of an output sequence $=3$). The candidate output sequences are $\mathit{A}$, $\mathit{C}$, $\mathit{AB}$, $\mathit{CE}$, $\mathit{ABD}$, and $\mathit{CED}$.](../img/beam-search.svg){#fig-beam-search}


@fig-beam-search demonstrates the 
...
$$\begin{aligned}P(A, y_2 \mid \mathbf{c}) = P(A \mid \mathbf{c})P(y_2 \mid A, \mathbf{c}),\\ P(C, y_2 \mid \mathbf{c}) = P(C \mid \mathbf{c})P(y_2 \mid C, \mathbf{c}),\end{aligned}$$   {#eq-beam-search-auto-2}
...
$$\begin{aligned}P(A, B, y_3 \mid \mathbf{c}) = P(A, B \mid \mathbf{c})P(y_3 \mid A, B, \mathbf{c}),\\P(C, E, y_3 \mid \mathbf{c}) = P(C, E \mid \mathbf{c})P(y_3 \mid C, E, \mathbf{c}),\end{aligned}$$  {#eq-beam-search-auto-3}
...
$$ \frac{1}{L^\alpha} \log P(y_1, \ldots, y_{L}\mid \mathbf{c}) = \frac{1}{L^\alpha} \sum_{t'=1}^L \log P(y_{t'} \mid y_1, \ldots, y_{t'-1}, \mathbf{c});$$ {#eq-beam-search-score}
...
in the summation of @eq-beam-search-score,
...
1. Apply beam search in the machine translation problem in @sec-seq2seq. How does the beam size affect the translation results and the prediction speed?
1. We used language modeling for generating text following  user-provided prefixes in @sec-rnn-scratch. Which kind of search strategy does it use? Can you improve it?
