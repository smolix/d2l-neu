# Gated Recurrent Units (GRU) {#sec-gru}


As RNNs and particularly the LSTM architecture (@sec-lstm)
...
to compute [@Chung.Gulcehre.Cho.ea.2014].
...
@fig-gru-1 illustrates the inputs for both
...
![Computing the reset gate and the update gate in a GRU model.](../img/gru-1.svg){#fig-gru-1}
...
in @eq-rnn-h-with-state,
leading to the following
*candidate hidden state*
$\tilde{\mathbf{H}}_t \in \mathbb{R}^{n \times h}$ at time step $t$:

$$\tilde{\mathbf{H}}_t = \tanh(\mathbf{X}_t \mathbf{W}_{\textrm{xh}} + \left(\mathbf{R}_t \odot \mathbf{H}_{t-1}\right) \mathbf{W}_{\textrm{hh}} + \mathbf{b}_\textrm{h}),$$ {#eq-gru-tilde-H}
...
Comparing with @eq-rnn-h-with-state,
...
we recover a vanilla RNN such as that in @eq-rnn-h-with-state.
...
@fig-gru-2 illustrates the computational flow after applying the reset gate.

![Computing the candidate hidden state in a GRU model.](../img/gru-2.svg){#fig-gru-2}
...
@fig-gru-3 shows the computational flow after the update gate is in action.

![Computing the hidden state in a GRU model.](../img/gru-3.svg){#fig-gru-3}
...
works in exactly the same manner as in @sec-rnn-scratch.
