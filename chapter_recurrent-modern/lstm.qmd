...
[@elman1990finding], the problems of learning long-term dependencies
...
due to @Hochreiter.Schmidhuber.1997 . 
...
![Computing the input gate, the forget gate, and the output gate in an LSTM model.](../img/lstm-0.svg){#fig-lstm-0}
...
$$\tilde{\mathbf{C}}_t = \textrm{tanh}(\mathbf{X}_t \mathbf{W}_{\textrm{xc}} + \mathbf{H}_{t-1} \mathbf{W}_{\textrm{hc}} + \mathbf{b}_\textrm{c}),$$ {#eq-lstm-auto-1}
...
A quick illustration of the input node is shown in @fig-lstm-1.

![Computing the input node in an LSTM model.](../img/lstm-1.svg){#fig-lstm-1}
...
$$\mathbf{C}_t = \mathbf{F}_t \odot \mathbf{C}_{t-1} + \mathbf{I}_t \odot \tilde{\mathbf{C}}_t.$$ {#eq-lstm-auto-2}
...
We thus arrive at the flow diagram in @fig-lstm-2.

![Computing the memory cell internal state in an LSTM model.](../img/lstm-2.svg){#fig-lstm-2}
...
$$\mathbf{H}_t = \mathbf{O}_t \odot \tanh(\mathbf{C}_t).$$ {#eq-lstm-auto-3}
...
to values close to 1. @fig-lstm-3 has a graphical illustration of the data flow.

![Computing the hidden state in an LSTM model.](../img/lstm-3.svg){#fig-lstm-3}


## Implementation from Scratch

Now let's implement an LSTM from scratch.
As same as the experiments in @sec-rnn-scratch,
...
Let's train an LSTM model by instantiating the `RNNLMScratch` class from @sec-rnn-scratch.
