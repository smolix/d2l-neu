...
# Machine Translation and the Dataset {#sec-machine-translation}
...
and much of @sec-chap-attention-and-transformers.
...
had been popular [@Brown.Cocke.Della-Pietra.ea.1988; @Brown.Cocke.Della-Pietra.ea.1990],
...
Unlike the language modeling that we saw in @sec-language-model,
...
## Loading Sequences of Fixed Length {#sec-loading-seq-fixed-len}
...
(number of time steps or tokens) argument from @sec-language-model.
...
As we will explain later (@fig-seq2seq),
...
for predicting the target sequence (@fig-seq2seq-predict).
