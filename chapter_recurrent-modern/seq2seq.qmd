...
#  Sequence-to-Sequence Learning for Machine Translation {#sec-seq2seq}

In so-called sequence-to-sequence problems such as machine translation
(as discussed in @sec-machine-translation),
...
Later, in @sec-chap-attention-and-transformers,
...
@fig-seq2seq illustrates
how to use two RNNs
for sequence-to-sequence learning
in machine translation.


![Sequence-to-sequence learning with an RNN encoder and an RNN decoder.](../img/seq2seq.svg){#fig-seq2seq}
...
into the decoder
at every single decoding time step [@Cho.Van-Merrienboer.Gulcehre.ea.2014].
In some other designs,
such as that of @Sutskever.Vinyals.Le.2014,
...
"Ils", "regardent", ".", "&lt;eos&gt;" (@fig-seq2seq).

Our implementation in
@sec-loading-seq-fixed-len
...
into a fixed-shape *context variable* $\mathbf{c}$ (see @fig-seq2seq).
...
$$\mathbf{h}_t = f(\mathbf{x}_t, \mathbf{h}_{t-1}). $$ {#eq-seq2seq-auto-1}
...
$$\mathbf{c} =  q(\mathbf{h}_1, \ldots, \mathbf{h}_T).$$ {#eq-seq2seq-auto-2}

For example, in @fig-seq2seq,
...
$$\mathbf{s}_{t^\prime} = g(y_{t^\prime-1}, \mathbf{c}, \mathbf{s}_{t^\prime-1}).$$ {#eq-seq2seq-s-t}
...
are summarized in @fig-seq2seq-details.

![Layers in an RNN encoder--decoder model.](../img/seq2seq-details.svg){#fig-seq2seq-details}
...
Recall from @sec-machine-translation
...
is illustrated in @fig-seq2seq-predict.
When the end-of-sequence ("&lt;eos&gt;") token is predicted,
the prediction of the output sequence is complete.


![Predicting the output sequence token by token using an RNN encoder--decoder.](../img/seq2seq-predict.svg){#fig-seq2seq-predict}

In the next section, we will introduce 
more sophisticated strategies 
based on beam search (@sec-beam-search).
...
machine translation results [@Papineni.Roukos.Ward.ea.2002],
has been extensively used in measuring
the quality of output sequences for different applications.
In principle, for any $n$-gram (@sec-markov-models-and-n-grams) in the predicted sequence,
...
$$ \exp\left(\min\left(0, 1 - \frac{\textrm{len}_{\textrm{label}}}{\textrm{len}_{\textrm{pred}}}\right)\right) \prod_{n=1}^k p_n^{1/2^n},$$ {#eq-bleu}

where $k$ is the longest $n$-gram for matching.

Based on the definition of BLEU in @eq-bleu,
...
the coefficient before the multiplication term in @eq-bleu
