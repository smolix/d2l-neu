...
If you completed the exercises in @sec-rnn-scratch,
...
when we introduced MLPs in @sec-backprop,
...
## Analysis of Gradients in RNNs {#sec-bptt-analysis}
...
$$\begin{aligned}h_t &= f(x_t, h_{t-1}, w_\textrm{h}),\\o_t &= g(h_t, w_\textrm{o}),\end{aligned}$$ {#eq-bptt-ht-ot}
...
$$\begin{aligned}\frac{\partial L}{\partial w_\textrm{h}}  & = \frac{1}{T}\sum_{t=1}^T \frac{\partial l(y_t, o_t)}{\partial w_\textrm{h}}  \\& = \frac{1}{T}\sum_{t=1}^T \frac{\partial l(y_t, o_t)}{\partial o_t} \frac{\partial g(h_t, w_\textrm{o})}{\partial h_t}  \frac{\partial h_t}{\partial w_\textrm{h}}.\end{aligned}$$ {#eq-bptt-partial-L-wh}

The first and the second factors of the
product in @eq-bptt-partial-L-wh
are easy to compute.
...
$$\frac{\partial h_t}{\partial w_\textrm{h}}= \frac{\partial f(x_{t},h_{t-1},w_\textrm{h})}{\partial w_\textrm{h}} +\frac{\partial f(x_{t},h_{t-1},w_\textrm{h})}{\partial h_{t-1}} \frac{\partial h_{t-1}}{\partial w_\textrm{h}}.$$ {#eq-bptt-partial-ht-wh-recur}
...
$$a_{t}=b_{t}+\sum_{i=1}^{t-1}\left(\prod_{j=i+1}^{t}c_{j}\right)b_{i}.$$ {#eq-bptt-at}
...
the gradient computation in @eq-bptt-partial-ht-wh-recur satisfies
$a_{t}=b_{t}+c_{t}a_{t-1}$.
Thus, per @eq-bptt-at, 
we can remove the recurrent computation 
in @eq-bptt-partial-ht-wh-recur with

$$\frac{\partial h_t}{\partial w_\textrm{h}}=\frac{\partial f(x_{t},h_{t-1},w_\textrm{h})}{\partial w_\textrm{h}}+\sum_{i=1}^{t-1}\left(\prod_{j=i+1}^{t} \frac{\partial f(x_{j},h_{j-1},w_\textrm{h})}{\partial h_{j-1}} \right) \frac{\partial f(x_{i},h_{i-1},w_\textrm{h})}{\partial w_\textrm{h}}.$$ {#eq-bptt-partial-ht-wh-gen}
...
One idea might be to compute the full sum in @eq-bptt-partial-ht-wh-gen.
...
backpropgation through time [@Jaeger.2002].
...
in @eq-bptt-partial-ht-wh-recur
with
...
![Comparing strategies for computing gradients in RNNs. From top to bottom: randomized truncation, regular truncation, and full computation.](../img/truncated-bptt.svg){#fig-truncated-bptt}


@fig-truncated-bptt illustrates the three strategies 
...
In contrast to the analysis in @sec-bptt-analysis,
...
\mathbf{o}_t &= \mathbf{W}_\textrm{qh} \mathbf{h}_{t},\end{aligned}$$ {#eq-bptt-auto-1}
...
$$L = \frac{1}{T} \sum_{t=1}^T l(\mathbf{o}_t, y_t).$$ {#eq-bptt-auto-2}
...
![Computational graph showing dependencies for an RNN model with three time steps. Boxes represent variables (not shaded) or parameters (shaded) and circles represent operators.](../img/rnn-bptt.svg){#fig-rnn-bptt}

As just mentioned, the model parameters in @fig-rnn-bptt 
...
According to the dependencies in @fig-rnn-bptt,
...
$$\frac{\partial L}{\partial \mathbf{o}_t} =  \frac{\partial l (\mathbf{o}_t, y_t)}{T \cdot \partial \mathbf{o}_t} \in \mathbb{R}^q.$$ {#eq-bptt-partial-L-ot}
...
is given by @eq-bptt-partial-L-ot.

Next, as shown in @fig-rnn-bptt,
...
$$\frac{\partial L}{\partial \mathbf{h}_T} = \textrm{prod}\left(\frac{\partial L}{\partial \mathbf{o}_T}, \frac{\partial \mathbf{o}_T}{\partial \mathbf{h}_T} \right) = \mathbf{W}_\textrm{qh}^\top \frac{\partial L}{\partial \mathbf{o}_T}.$$ {#eq-bptt-partial-L-hT-final-step}
...
as discussed in @sec-bptt-analysis. 
...
@eq-bptt-partial-L-hT-final-step
and @eq-bptt-partial-L-ht-recur
...
as we have explained in @sec-backprop,
