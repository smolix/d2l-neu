...
In @sec-text-sequence, we saw how to map text sequences into tokens, where these tokens can be viewed as a sequence of discrete observations such as words or characters. Assume that the tokens in a text sequence of length $T$ are in turn $x_1, x_2, \ldots, x_T$.
The goal of *language models*
is to estimate the joint probability of the whole sequence:

$$P(x_1, x_2, \ldots, x_T),$$ {#eq-language-model-auto-1}
...
$$P(x_1, x_2, \ldots, x_T) = \prod_{t=1}^T P(x_t  \mid  x_1, \ldots, x_{t-1}).$$ {#eq-language-model-auto-2}
...
=&P(\textrm{deep}) P(\textrm{learning}  \mid  \textrm{deep}) P(\textrm{is}  \mid  \textrm{deep}, \textrm{learning}) P(\textrm{fun}  \mid  \textrm{deep}, \textrm{learning}, \textrm{is}).\end{aligned}$$ {#eq-language-model-auto-3}

### Markov Models and $n$-grams {#sec-markov-models-and-n-grams}

Among those sequence model analyses in @sec-sequence,
...
$$\hat{P}(\textrm{learning} \mid \textrm{deep}) = \frac{n(\textrm{deep, learning})}{n(\textrm{deep})},$$ {#eq-language-model-auto-4}
...
As suggested by the empirical results in @sec-natural-lang-stat,
...
\end{aligned}$$ {#eq-language-model-auto-5}
...
First, 
as discussed in @sec-natural-lang-stat,
...
$$\frac{1}{n} \sum_{t=1}^n -\log P(x_t \mid x_{t-1}, \ldots, x_1),$$ {#eq-avg-ce-for-lm}

where $P$ is given by a language model and $x_t$ is the actual token observed at time step $t$ from the sequence.
This makes the performance on documents of different lengths comparable. For historical reasons, scientists in natural language processing prefer to use a quantity called *perplexity*. In a nutshell, it is the exponential of @eq-avg-ce-for-lm:
...
![Obtaining five pairs of input sequences and target sequences from partitioned length-5 subsequences.](../img/lang-model-data.svg){#fig-lang-model-data}

@fig-lang-model-data shows an example of obtaining five pairs of input sequences and target sequences with $n=5$ and $d=2$.
...
Language models can be scaled up with increased data size, model size, and amount in training compute. Large language models can perform desired tasks by predicting output text given input text instructions. As we will discuss later (e.g., @sec-large-pretraining-transformers),
