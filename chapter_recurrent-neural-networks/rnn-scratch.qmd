# Recurrent Neural Network Implementation from Scratch {#sec-rnn-scratch}
...
The model here uses a $\tanh$ activation function (@sec-tanh).
...
As discussed in @sec-perplexity, this ensures 
...
(recall from @sec-classification-problem).
...
As mentioned in @sec-numerical-stability, 
...
$$|f(\mathbf{x}) - f(\mathbf{y})| \leq L \|\mathbf{x} - \mathbf{y}\|.$$ {#eq-rnn-scratch-auto-1}
...
$$|f(\mathbf{x}) - f(\mathbf{x} - \eta\mathbf{g})| \leq L \eta\|\mathbf{g}\|.$$ {#eq-rnn-scratch-auto-2}
...
$$\mathbf{g} \leftarrow \min\left(1, \frac{\theta}{\|\mathbf{g}\|}\right) \mathbf{g}.$$ {#eq-rnn-scratch-auto-3}
...
the `d2l.Trainer` class (see @sec-linear-scratch).
