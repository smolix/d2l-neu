# Working with Sequences {#sec-sequence}
...
from the FTSE 100 index (@fig-ftse100).
At each *time step* $t \in \mathbb{Z}^+$, we observe
the price, $x_t$, of the index at that time.


![FTSE 100 index over about 30 years.](../img/ftse100.png){#fig-ftse100 width="400px"}
...
$$P(x_t \mid x_{t-1}, \ldots, x_1)$$ {#eq-sequence-auto-1}
...
$$\mathbb{E}[(x_t \mid x_{t-1}, \ldots, x_1)],$$ {#eq-sequence-auto-2}

would be to apply a linear regression model
(recall @sec-linear-regression).
...
![A latent autoregressive model.](../img/sequence-model.svg){#fig-sequence-model}
...
$$P(x_1, \ldots, x_T) = P(x_1) \prod_{t=2}^T P(x_t \mid x_{t-1}, \ldots, x_1).$$ {#eq-sequence-auto-3}
...
### Markov Models {#sec-markov-models}
...
$$P(x_1, \ldots, x_T) = P(x_1) \prod_{t=2}^T P(x_t \mid x_{t-1}).$$ {#eq-sequence-auto-4}
...
$$P(x_1, \ldots, x_T) = P(x_T) \prod_{t=T-1}^1 P(x_t \mid x_{t+1}, \ldots, x_T).$$ {#eq-sequence-auto-5}
...
&\vdots\end{aligned}$$ {#eq-sequence-auto-6}
