...
* It may happen that we do not know how the robot moves *exactly* but only know it up to some approximation. We model this situation in reinforcement learning as follows: if the robot takes an action "go forward", there might be a small probability that it stays at the current state, another small probability that it "turns left", etc. Mathematically, this amounts to defining a "transition function" $P: \mathcal{S} \times \mathcal{A} \times \mathcal{S} \to [0,1]$ where $P(s' \mid s, a)$ is the conditional probability of reaching a state $s'$ given that the robot was at state $s$ and took an action $a$. The transition function is a probability distribution and we therefore have $\sum_{s' \in \mathcal{S}} P(s' \mid s, a) = 1$ for all $s \in \mathcal{S}$ and $a \in \mathcal{A}$, i.e., the robot has to go to some state if it takes an action.
...
Note that if $\gamma$ is very small, the rewards earned by the robot in the far future, say $t = 1000$, are heavily discounted by the factor $\gamma^{1000}$. This makes the robot *myopic*: it prefers short trajectories that achieve its goal quickly, namely that of going to the green house in the gridworld example (see :numref:`fig_mdp`). For values of the discount factor close to one, say $\gamma = 0.99$, rewards earned far in the future are discounted only mildly, so the robot is *far-sighted*: it is willing to accept a longer trajectory, or forgo small immediate rewards, if doing so leads to a larger total reward in the end.
...
2. How would you design an MDP for an Atari game like [Pong game](https://ale.farama.org/environments/pong/)?
...
$$s_t \xrightarrow{\text{agent chooses } a_t}
  (s_t, a_t) \xrightarrow{\text{environment yields}} (r_t, s_{t+1}).$$
...
together with a discount factor $\gamma$.
