...
where the summation runs over all possible trajectories of length $T$. The fixed $T$ is a bookkeeping device: pad every episode that ends early with the absorbing terminal state of :numref:`sec_mdp`, at zero reward, and all trajectories share a common horizon while the padded steps change nothing below, their rewards being zero and their scores averaging to zero given the prefix (a fact :numref:`sec_baselines` proves and then turns into a tool), so the code may keep only the recorded episode. Note that $J(\theta) = V^{\pi_\theta}(s_0)$ in the notation of :numref:`sec_valueiter` (our lake, with the time limit stripped, is episodic: every trajectory reaches a terminal state with probability one, so for $T$ past the episode's length the finite sum agrees with the infinite-horizon return); the new point of view is that we can search for good parameters directly,
...
which is an unbiased estimate of $\nabla_\theta J(\theta)$, and then takes a gradient ascent step $\theta \leftarrow \theta + \alpha \hat{u}$ with learning rate $\alpha$. This algorithm is known as REINFORCE :cite:`Williams.1992`. It has a simple interpretation: each term pushes on the log-probability of the actions taken in trajectory $\tau_i$, with that trajectory's return as the coefficient. Read the direction with care: a sampled action's own log-probability is pushed *down* only when its weight is negative, which cannot happen on this lake, where every return is $0$ or positive. With all-positive weights every sampled action is pushed up, harder on better trajectories, and an action's probability falls only through the softmax coupling, when competing samples at the same state are pushed harder; a weight with a meaningful zero point, positive for better than usual and negative for worse, is what :numref:`sec_baselines` constructs. Note that after every parameter update the policy changes, so the agent must collect fresh trajectories from the new policy before the next update. :numref:`fig_rl_score_ascent` draws one such step on the smallest possible instance, every arrow pointing up and only the sizes sorting good actions from bad.
...
One note on fidelity: :eqref:`eq_reinforce` sums each trajectory's scores and divides by the number of trajectories $n$, while `policy_step` averages over all *steps* in the batch. The two differ by a factor of the batch's realized mean episode length, and that factor is random and correlated with how well the policy is doing, so the implemented update is a step-averaged variant of :eqref:`eq_reinforce`, not the estimator the derivation covers: on any single batch the difference is a positive rescale of the same direction, but across batches it reweights them. We keep the variant because it is what practice runs, :numref:`sec_baselines` measures all four divisor choices side by side, and the yardstick cells below compute :eqref:`eq_reinforce` itself, dividing by $n$. Three seeds, and the curve each run yields is the estimate of $J(\theta)$ itself, computed from the very trajectories that drove the updates, so it costs nothing extra:
...
- calm ice, and no time limit, both named as assumptions: the estimator
  is the subject, the environment is scenery
:::
