...
As we discussed, implementing this algorithm requires knowing the MDP, specifically the transition function $P(s' \mid s, a)$. The key idea behind Q-Learning is to replace the summation over all $s' \in \mathcal{S}$ in the above expression by a summation over the states visited by the robot. This allows us to sidestep the need to know the transition function.

## An Optimization Problem Underlying Q-Learning

Let us imagine that the robot uses a policy $\pi_e(a \mid s)$ to take actions and collects a dataset of $n$ trajectories of $T$ timesteps each. Each trajectory records the states visited, the actions taken, and the rewards received along the way, so the dataset consists of tuples $\{ (s_t^i, a_t^i, r_t^i, s_{t+1}^i)_{t=0,\ldots,T-1}\}_{i=1,\ldots, n}$, where $r_t^i$ is the observed reward for taking action $a_t^i$ at state $s_t^i$. Recall that value iteration is really a set of constraints that ties together the action-value $Q^*(s, a)$ of different states and actions to each other. We can implement an approximate version of value iteration using the data that the robot has collected using $\pi_e$ as

$$\hat{Q} = \underset{Q}{\mathrm{argmin}} \underbrace{\frac{1}{nT} \sum_{i=1}^n \sum_{t=0}^{T-1} (Q(s_t^i, a_t^i) - r_t^i - \gamma \max_{a'} Q(s_{t+1}^i, a'))^2}_{\stackrel{\textrm{def}}{=} \ell(Q)}.$$
:eqlabel:`q_learning_optimization_problem`

Let us first observe the similarities and differences between this expression and value iteration above. If the robot's policy $\pi_e$ visited every state-action pair often enough, and if it collected an infinite amount of data, then in the limit the constraints in this optimization problem would coincide with the constraints that value iteration enforces. But while value iteration requires us to know $P(s' \mid s, a)$, the optimization objective does not have this term. We have not cheated: as the robot uses the policy $\pi_e$ to take an action $a_t^i$ at state $s_t^i$, the next state $s_{t+1}^i$ is a sample drawn from the transition function. So the optimization objective also has access to the transition function, but implicitly in terms of the data collected by the robot.

The variables of our optimization problem are $Q(s, a)$ for all $s \in \mathcal{S}$ and $a \in \mathcal{A}$. We can minimize the objective with updates of a gradient-descent flavor. For every tuple $(s_t^i, a_t^i, r_t^i, s_{t+1}^i)$ in our dataset, we take a step on the corresponding squared-error term while treating the target $r_t^i + \gamma \max_{a'} Q(s_{t+1}^i, a')$ as a constant, i.e., we do not differentiate through the $Q$ inside the target. Folding the remaining constant factors into the learning rate $\alpha$, the update becomes

$$Q(s_t^i, a_t^i) \leftarrow (1 - \alpha) Q(s_t^i,a_t^i) + \alpha \Big( r_t^i + \gamma \max_{a'} Q(s_{t+1}^i, a') \Big),$$
:eqlabel:`q_learning`

where $\alpha$ is the learning rate. Because the target itself depends on $Q$ and we deliberately ignore that dependence, this is called a *semi-gradient* update: Q-Learning is not gradient descent on $\ell(Q)$ in the strict sense, but this update is simple, cheap, and works remarkably well. Typically in real problems, when the robot reaches the goal location, the trajectories end. The value of such a terminal state is zero because the robot does not take any further actions beyond this state. We should modify our update to handle such states as

$$Q(s_t^i, a_t^i) \leftarrow (1 - \alpha) Q(s_t^i,a_t^i) + \alpha \Big( r_t^i + \gamma (1 - \mathbb{1}_{s_{t+1}^i \textrm{ is terminal}} )\max_{a'} Q(s_{t+1}^i, a') \Big),$$

where $\mathbb{1}_{s_{t+1}^i \textrm{ is terminal}}$ is an indicator variable that is one if $s_{t+1}^i$ is a terminal state and zero otherwise. The values $Q(s, a)$ of all state-action pairs are initialized to zero; pairs that never appear in the dataset simply retain this initial value. This algorithm is known as Q-Learning.

Given the solution of these updates $\hat{Q}$, which is an approximation of the optimal action-value function $Q^*$, we can obtain the optimal deterministic policy corresponding to this value function easily using
...
$$\pi_e(a \mid s) = \begin{cases} 1-\epsilon + \epsilon/|\mathcal{A}| & \textrm{if } a = \mathrm{argmax}_{a'} \hat{Q}(s, a') \\ \epsilon/|\mathcal{A}| & \textrm{otherwise,}\end{cases}$$
:eqlabel:`epsilon_greedy`

where $\epsilon$ is called the "exploration parameter" and is chosen by the user. The policy $\pi_e$ is called an exploration policy. This particular $\pi_e$ is called an $\epsilon$-greedy exploration policy: with probability $\epsilon$ it picks an action uniformly at random from $\mathcal{A}$, and otherwise it picks the greedy action, i.e., the best action under the current estimate $\hat{Q}$ (so the greedy action is chosen with total probability $1-\epsilon + \epsilon/|\mathcal{A}|$). We can also use the so-called softmax exploration policy

$$\pi_e(a \mid s) = \frac{e^{\hat{Q}(s, a)/\tau}}{\sum_{a'} e^{\hat{Q}(s, a')/\tau}};$$

where the hyper-parameter $\tau$ is called temperature (we use $\tau$ to avoid clashing with the trajectory length $T$ introduced above). A large value of $\epsilon$ in the $\epsilon$-greedy policy functions similarly to a large value of temperature $\tau$ for the softmax policy.
...
In the FrozenLake environment, the robot moves on a $4 \times 4$ grid (these are the states) with actions that are "up" ($\uparrow$), "down" ($\downarrow$), "left" ($\leftarrow$), and "right" ($\rightarrow$). The environment contains a number of hole (H) cells and frozen (F) cells as well as a goal cell (G), all of which are unknown to the robot. To keep the problem simple, we assume the robot has reliable actions, i.e., the transitions are deterministic: each action moves the robot to the intended neighboring cell with probability one. If the robot reaches the goal, the trial ends and the robot receives a reward of $1$; the reward everywhere else is $0$ for all actions. The objective of the robot is to learn a policy that reaches the goal location (G) from a given start location (S) (this is $s_0$) to maximize the *return*.

We first implement the $\epsilon$-greedy method. When exploiting, we break ties among equally-valued actions uniformly at random: `np.argmax` alone always returns the *first* maximizing index, so with a zero-initialized $Q$-table every unvisited state would deterministically pick action 0 ("left") — a systematic bias that can prevent the robot from ever finding the goal:
...
def e_greedy(env, Q, s, epsilon):
    ...
    # Exploit: greedy action, breaking ties uniformly at random
...
This result shows that Q-learning finds a policy that reaches the goal, and it does so from experience alone — after a few dozen episodes the greedy policy already reaches the goal, and the value estimates continue to be refined over the remaining episodes. However, when we compare this result with the Value Iteration algorithm's result (see :numref:`subsec_valueitercode`), we can see that Value Iteration converges in far fewer of its (full-sweep) iterations than the number of episodes Q-learning needs. This happens because the Value Iteration algorithm has access to the full MDP whereas Q-learning has to discover it through interaction, one trajectory at a time. Note also that Q-learning pins down accurate values only for the states that the exploration policy visits often; states far off the greedy path may retain rougher estimates.
...
1. Run the Q-learning algorithm again with the discount factor $\gamma$ (i.e., `gamma` in the above code) set to $0$, $0.5$, and $1$, and analyze the results.
...
\epsilon/|\mathcal{A}|, & \textrm{otherwise.}
\end{cases}
$$
