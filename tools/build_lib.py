...
def build_library(src_dir, output_file, framework, lib_config, files):
    ...
    # Phase-2: also write per-symbol shards under d2l/_blocks/<fw>/<sym>.py.
    # These aren't imported at runtime — they're dependency anchors for the
    # Make build system so that a per-notebook `.executed` stamp can pin to
    # the exact symbols it consumes. Each shard is content-aware so an
    # unrelated edit elsewhere doesn't bump its mtime.
    fw_name = output_path.stem  # 'torch', 'tensorflow', 'jax', 'mxnet'
    # Map framework file stems back to the framework tag stored in MANIFEST.
    stem_to_fw = {'torch': 'pytorch', 'tensorflow': 'tensorflow',
                  'jax': 'jax', 'mxnet': 'mxnet'}
    fw_tag = stem_to_fw.get(fw_name, fw_name)
    shard_dir = output_path.parent / '_blocks' / fw_tag
    shard_dir.mkdir(parents=True, exist_ok=True)

    # Collect new symbols and their content.
    seen_shards = set()
    n_shard_written = 0
    for code, label, source in all_blocks:
        names = _block_defined_names(code)
        if not names:
            continue
        # Pick the first defined name as the primary symbol; multi-name
        # blocks (rare, e.g. tuple assignments) collapse to one shard.
        primary = sorted(names)[0]
        shard_path = shard_dir / f'{primary}.py'
        seen_shards.add(shard_path.name)
        new = (code + '\n').encode('utf-8')
        if shard_path.exists() and shard_path.read_bytes() == new:
            continue
        shard_path.write_bytes(new)
        n_shard_written += 1

    # Prune stale shards (symbols that no longer exist in this framework).
    for old in shard_dir.glob('*.py'):
        if old.name not in seen_shards:
            old.unlink()

    if n_shard_written:
        print(f'  Wrote {n_shard_written} shard(s) under {shard_dir}/')

    # Per-framework MANIFEST.mk lists every shard so Make knows the symbol
    # universe. Used by the per-notebook .d files generated downstream.
    shard_files = sorted(shard_dir.glob('*.py'))
    manifest = (f'# Auto-generated by tools/build_lib.py — do not edit.\n'
                f'D2L_SHARDS_{fw_tag} := '
                + ' '.join(str(p) for p in shard_files) + '\n')
    manifest_path = shard_dir / 'MANIFEST.mk'
    new_manifest = manifest.encode('utf-8')
    if not (manifest_path.exists() and manifest_path.read_bytes() == new_manifest):
