...
TEAL = fl.T.TEAL.base      # state role (guide §4.3)
PURPLE = fl.T.PURPLE.base  # parameter role
RED = fl.T.RED.base        # loss / gradient role
...
def _panel(ax, pretraining):
    ...
    for x, lab in zip(xs, labels):
        _box(ax, x, y_state, sw, sh, lab, TEAL, fontsize=14)
    for xa, xb in zip(xs[:-1], xs[1:-1]):   # arrows between drawn states
        fl.arrow(ax, (xa + sw / 2, y_state),
                 (xb - sw / 2, y_state), color=GRAY, lw=1.9, mut=13)
    ax.text((xs[2] + xs[3]) / 2, y_state, r"$\cdots$", fontsize=16,
            color=GRAY, ha="center", va="center")
    fl.arrow(ax, ((xs[2] + xs[3]) / 2 + 0.42, y_state),
             (xs[3] - sw / 2, y_state), color=GRAY, lw=1.9, mut=13)
    ax.plot([xs[2] + sw / 2, (xs[2] + xs[3]) / 2 - 0.42],
    ...
    for x, sub in zip(xs[1:], ["1", "2", "T"]):
        fl.arrow(ax, (x, 1.18), (x, y_state - sh / 2), color=GRAY,
                 lw=1.6, mut=11)
    ...
    if pretraining:
        _box(ax, th_cx, th_cy, th_w, th_h,
             ...
             r" gates, $\mathbf{S}_0$",
             PURPLE, fontsize=12.5)
    else:
        _box(ax, th_cx, th_cy, th_w, th_h,
             "outer parameters $\\theta$\nfrozen", GRAY, fontsize=12.5)
    # θ parameterizes every inner update: the arrow lands ON the S_0 -> S_1
    # transition (the update it parameterizes)
    fl.arrow(ax, (th_cx, th_cy - th_h / 2),
             (th_cx, y_state + 0.04),
             color=PURPLE if pretraining else GRAY, lw=1.8, mut=12)

    # ── read-out and, in pretraining, the loss + outer gradient ─────────
    fl.arrow(ax, (7.85, y_state + sh / 2), (7.85, 5.92 - 0.50),
    ...
    if pretraining:
        ...
        fl.arrow(ax, (7.85 - 2.95 / 2, 5.92),
                 (th_cx + th_w / 2, 5.92),
                 color=RED, lw=2.2, ls="--", mut=15)
        ax.text(5.32, 4.97,
